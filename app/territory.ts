...
export type Territory = {categories:Uint8Array;regions:Uint8Array;searchMarks:Uint8Array;searchTag:number};
...
export function markBuildingTerritory(land:Territory,b:TerritoryBuilding,radius:number,remove=false) {
  const widths=rules.territoryWidths[radius===5?0:radius===7?1:radius===9?2:3];
  const extent=widths.length-1,bit=(1<<(b.tribe+4))&255;
...
export function refreshBuildingTerritory(land:Territory,turn:number,
  ...
  if(!land.searchTag){land.searchMarks.fill(0);land.searchTag=1;}
