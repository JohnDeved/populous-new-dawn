...
export function specialBattlePosition(w: BattleWorld, rank: number, tribe: number) {
  let angle = [0, 1024, 512, 1536][w.directions[tribe]],
    distance: number
  ...
  if (rank <= 0) {
    if (rank < 0) {
      angle = angle - (random(w) % 284) - 142
      distance = 3776
    } else {
      angle = (angle - 142) & 2047
      distance = 3392
    }
  } else {
    const width = Math.max(6, Math.trunc(w.populations[tribe] / 8)),
      row = Math.trunc((rank - 1) / width)
    step(((rank - 1) % width) * Math.trunc(170 / width) + angle - 85, 2560)
    distance = short(row * 160)
  }
  step(angle, distance)
  return point
}
...
export function initializeSpecialBattle(
  w: BattleWorld,
  p: BattlePerson,
  effects: {
    leaveVehicle: () => void
    ...
    initializeEffect: (effect: BattleEffect) => void
    releaseMotion: () => void
  ...
  if (p.model === 7) {
    ...
    effects.insert({ ...specialBattlePosition(w, short(p.link), p.tribe), h: 0 })
    p.h = effects.height(p)
    p.flags4 = (p.flags4 & ~0x400) >>> 0
    p.displacement.x = p.displacement.y = p.displacement.h = 0
    ...
    if (effect) {
      ...
      if (!(effect.flags2 & 0x100000)) {
        effect.state = 36
        effects.initializeEffect(effect)
      }
      effect.duration = 15
      setAnimationObject(effect, 44, 1401)
...
export function enforceSpecialBattleBoundary(
  center: Point & { substate: number; delay: number },
  p: Point & { life: number }
) {
  if (center.substate > 1 && !center.delay) {
    const dx = short(center.x - p.x),
      dy = short(center.y - p.y)
    if (((dx * dx + dy * dy) | 0) > 0x1440000) p.life = 0
  }
}
