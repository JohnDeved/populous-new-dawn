...
export function HudSprite({id}:{id:number|string}){
  ...
  return <i className="hud-sprite" aria-hidden="true" style={{width:r.w,height:r.h,backgroundPosition:`-${r.x}px -${r.y}px`}}/>;
}

export function NativeText({text,font=0}:{text:string;font?:0|2}){
  return <span className="native-text" role="img" aria-label={text}>{Array.from(text,(c,i)=><HudSprite key={i} id={hudGlyph(c.charCodeAt(0),font)}/>)}</span>;
}
