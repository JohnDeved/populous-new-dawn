...
export type ManaPerson = Pick<OrderedPerson, 'model' | 'state' | 'flags2' | 'flags4' | 'assignment' | 'commandStatus' |
  'commands' | 'commandCursor' | 'immediateCommand'> & {class: number; tribe: number};
...
export function personMana(pool: OrderPool, p: ManaPerson) {
  ...
  const inside = !!(p.flags2 & 0x800000), commanded = !!(p.commandStatus & 255);
  ...
  if (!commanded || (p.model === 4 && executingPreacherOrder(pool,p) && !(p.assignment & 64))) return rules.manaIdleWarrior;
...
export function generatedMana(pool: OrderPool, people: ManaPerson[], tribes: Pick<ManaTribe,'playerType'>[]) {
  const totals = [0,0,0,0];
  for (const p of people) if (p.class === 1 && (p.flags4 & 0x20000000) && !(p.flags4 & 0x800) && p.model !== 1 && p.model !== 8) {
    totals[p.tribe] = (totals[p.tribe] + personMana(pool,p)) | 0;
  }
  return totals.map((n,i)=>divide(Math.imul(n,tribes[i].playerType===2?rules.humanManaFactor:rules.computerManaFactor),256));
}

// 0x4ecac0 generation gate and commit. Other counter/list rebuild work is separate.
export function generateFollowerMana(w: Pick<ManaWorld,'gameFlags'|'turn'>, tribes: ManaTribe[], people: ManaPerson[], pool: OrderPool) {
  ...
  tribes.forEach((t,i)=>{
    ...
    t.previousRate = totals[i]; t.estimatedRate = 0;
  });
}
