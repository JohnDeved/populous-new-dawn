...
export function configurePersonOrder(
  w: OrderStartWorld,
  p: StartingPerson,
  effects: OrderStartEffects
) {
  ...
  if (order && !(order.flags & 1)) {
    ...
    if (!(p.flags4 & 0x8000000) && p.vehicle && p.flags4 & 0x2000000) {
      const pos = effects.commandPosition(order)
      p.destinationX = pos.x
      p.destinationY = pos.y
    }
    if (!(flags & 0x40000)) p.flags4 = (p.flags4 & ~8) >>> 0
    else if (!(p.flags4 & 8) && (!p.vehicle || effects.allowVehicleOrder(p)))
      p.flags4 = (p.flags4 | 8) >>> 0
    if (order.model === 18) p.flags4 = (p.flags4 | 128) >>> 0
    else if ([6, 7, 10, 11, 19, 21, 25, 28, 30, 31].includes(order.model))
      effects.initializeCommand(p, order)
    if (flags & 1) {
      if (!(flags & 0x200000)) effects.setDestination(p, short(order.a), short(order.b))
    } else if (flags & 4) p.target = short(order.a)
    else if (flags & 0x800) {
      if (!(flags & 0x200000))
        effects.setDestination(p, short((order.a & 254) << 8), short(order.a & 0xfe00))
    } else if (flags & 0x242) p.target = short(order.a)
    p.commandStatus = order.model
    if (p.assignment & 32) {
      p.assignment &= ~32
      recoverPersonMovement(w, p, effects.setAnimation)
    }
    p.orderDelay = 8
    p.assignment = descriptor(order.model).flags & 16 ? p.assignment | 8 : p.assignment & ~8
  }
  if (!p.savedVehicle && p.vehicle && w.tribes[p.tribe].vehicleMode === 2)
    p.savedVehicle = p.vehicle
...
export function reconcileOrderBuilding(
  w: OrderStartWorld,
  p: StartingPerson,
  effects: OrderStartEffects
) {
  ...
  if (p.commandStatus === 8) {
    const order = currentPersonOrder(w.orders, p)
    if (order) {
      const building = effects.adjacentBuilding(p, 0)
      stay = !!building && order.a === building
    }
...
export function startPersonOrders(
  w: OrderStartWorld,
  p: StartingPerson,
  effects: OrderStartEffects
) {
  ...
  configurePersonOrder(w, p, effects)
  reconcileOrderBuilding(w, p, effects)
  if (p.flags3 & 128) {
    const order = currentPersonOrder(w.orders, p)
