...
export type ConversionWorld = Omit<OccupancyWorld, 'people'> & {
  people: Map<number, Trainee>
  playerTribe: number
}
...
export function stepTrainingConversion(
  w: ConversionWorld,
  b: ConvertingBuilding,
  effects: ConversionEffects
) {
  ...
  if (!(b.activity & 128)) {
    ...
    if (head && head.model !== trained && head.model !== 7)
      removeBuildingOccupant(w, b, first, effects)
    let queued = live(w, b.queueHead),
      untrained = false
    ...
    if (untrained) {
      ...
      while (queued?.model === trained) {
        queued.substate = 0
        queued.flags2 = (queued.flags2 | 0x40000000) >>> 0
        queued = live(w, queued.reservationNext)
      }
      b.activity |= 0x2000
      removeBuildingOccupant(w, b, first, effects)
  ...
  const conversions = Math.trunc(weight / divisor),
    remainder = weight - conversions * divisor
  if (!repriced) repriceTraining(w, b)
  ...
  for (let i = 0; i < count; i++) {
    const p = effects.allocateTrainee(
      i < conversions ? trained : 2,
      b.tribe,
      inside.x,
      inside.y,
      b.angle
