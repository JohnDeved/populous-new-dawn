...
export type CelebrationWorld = {
  randomState: number
  people: Map<number, Celebrant>
  shamans: number[]
...
export type CelebrationEffects = {
  ...
  releaseMotion: (p: Celebrant) => void
  destination: (p: Celebrant, to: { x: number; y: number }) => void
  dropLog: (p: Celebrant) => boolean
  sound: (p: Celebrant, cue: number) => void
  leaveBuilding: (p: Celebrant) => void
  projectile: (p: Celebrant, to: { x: number; y: number; h: number }) => void
}
const short = (n: number) => (n << 16) >> 16,
  byte = (n: number) => (n << 24) >> 24
...
const transition = (p: Celebrant, phase: number) => {
  p.substate = phase
  p.flags2 = (p.flags2 | 0x40000000) >>> 0
}
const enter = (p: Celebrant) => {
  const yes = !!(p.flags2 & 0x40000000)
  p.flags2 = (p.flags2 & ~0x40000000) >>> 0
  return yes
}
const step = (p: { x: number; y: number; h: number }, angle: number, length: number) => ({
  ...p,
...
function row(p: Celebrant, r: number) {
  if (p.flags2 & 0x80000) {
    r = p.flags4 & 0x400 ? 12 : 2
    if (r === 2) p.flags2 = (p.flags2 & ~0x8000) >>> 0
  }
...
function face(p: Celebrant, angle: number, e: CelebrationEffects, immediate = false) {
  ...
  if (immediate) {
    p.heading = p.turnAngle
    p.angle = (p.turnAngle + (p.flags2 & 0x8000 ? 1024 : 0)) & 2047
  }
}
...
export function setChainAction(p: Celebrant, action: number, e: CelebrationEffects) {
  if (action === 0 || action === 1) {
    ...
    e.animation(p, row(p, action === 0 ? (p.cargo ? 5 : 1) : p.cargo ? 4 : 0), true)
    p.f2 = 0
    p.f1 = 1
  } else if (action === 2 || action === 3) {
    p.speed = 0
    e.animation(p, rules.personAnimationObjects[7 * 9 + p.model], false)
    p.f2 = action === 2 ? 0 : 1
    p.renderFlags |= 2
    p.f1 = 0
  }
...
export function stepCelebration(w: CelebrationWorld, p: Celebrant, e: CelebrationEffects) {
  ...
  const faceShaman = () => {
    const s = w.people.get(w.shamans[p.tribe])
    if (!(p.counter & 3) && s) face(p, bearing(p, s), e)
  }
  switch (p.substate) {
    case 0:
      if (enter(p)) {
        ...
        while (cargo > 0) {
          if (!e.dropLog(p)) break
          cargo -= 100
          e.sound(p, 11)
        }
        p.cargo = short(Math.max(0, cargo))
        p.speed = 0
        e.animation(p, row(p, p.cargo ? 4 : 0), true)
        if (p.flags2 & 0x800000) {
          e.leaveBuilding(p)
          p.flags2 = (p.flags2 & ~16) >>> 0
        }
        transition(p, p.model === 7 ? 8 : p.vehicle ? 7 : 1)
        p.animationMode = 0
        p.assignment |= 16
      }
      break
    case 1: {
      ...
      if (p.animationMode === 0) {
        if (p.assignment & 16) {
          p.assignment &= ~16
          p.commandPhase = (random(w) & 7) + 8
          p.timer = p.commandPhase
          e.animation(p, rules.personAnimationObjects[25 * 9 + p.model], true)
          p.speed = randomPersonSpeed(w, p)
          newMove = true
        }
        moving = true
        p.commandPhase = byte(p.commandPhase - 1)
        finished = p.commandPhase < 1
      } else if (p.animationMode === 1) {
        if (p.assignment & 16) {
          p.assignment &= ~16
          p.commandPhase = (random(w) & 15) + 16
          dance(p, e)
        ...
        if (p.assignment & 16) {
          ...
          p.assignment &= ~16
          p.commandPhase = (random(w) & 15) + 16
          e.animation(p, rules.personAnimationObjects[3 * 9 + p.model], true)
        }
        faceShaman()
        p.commandPhase = byte(p.commandPhase - 1)
        finished = p.commandPhase < 1
      ...
      if (moving || newMove) {
        ...
        if (p.timer < 1 || newMove) {
          ...
          const a = bearing(p, { x: p.anchorX, y: p.anchorY })
          face(p, a + (random(w) % 1422) - 711, e)
        }
      }
      if (!finished && p.animationMode === 1) {
        let movingCount = 0,
          followers = 0,
          leader: Celebrant | undefined
        for (const q of here())
          if (q !== p && same(q)) {
        ...
        if (followers < 7 && (leader || movingCount > 2)) {
          p.speed = 0
          transition(p, leader ? 4 : 3)
        }
      }
      if (!finished) break
      const previous = p.commandAux
      p.commandAux = (previous + 1) & 255
      let noNeighbors = previous >= 4,
        mode = 0
      if (!noNeighbors) {
        const weights = rules.celebrationWeights.map(r => [...r])
        let count = 0
        for (const c of [
          ...Array.from({ length: 8 }, (_, i) => spiralCell(cell(p), i, 0)),
          cell(p),
        ])
          for (const q of w.cellPeople(c))
            if (q !== p && same(q) && q.substate === 1) {
              const dx = short(p.x - q.x),
                dy = short(p.y - q.y)
              if ((dx * dx + dy * dy) >>> 0 < 160001) {
                count++
                const r = weights[q.animationMode]
                r[0] = short(r[0] + r[2])
              }
            }
        noNeighbors = count === 0
      ...
      if (noNeighbors) {
        p.commandAux = 0
        if (p.animationMode === 0) mode = (random(w) & 7) < 4 ? 1 : 2
      }
      p.assignment |= 16
      p.animationMode = mode
    ...
    case 2:
      ...
      if (p.animationMode === 1) {
        if (p.assignment & 16) {
          p.speed = 0
          p.assignment &= ~16
          e.animation(p, row(p, 15), true)
          p.f2 = 0
          p.f1 = 1
          p.timer = short(e.animationTiming(p).duration)
          const target = step(p, p.heading, 1280)
          target.h = short(target.h + 3072)
          e.projectile(p, target)
        }
        p.timer = short(p.timer - 1)
        if (!p.timer) {
          p.animationMode = 2
        ...
        p.timer = short(p.timer - 1)
        if (!p.timer) transition(p, 1)
      }
      break
    case 3: {
      if (enter(p)) {
        p.commandPhase = (random(w) & 31) + 40
        p.timer = random(w) & 2047
        dance(p, e)
        p.speed = randomPersonSpeed(w, p)
      }
      let count = 0
      for (const q of here())
        if (same(q) && (q.substate === 3 || q.substate === 4)) {
          q.flags3 = (q.flags3 | 16) >>> 0
          count++
        }
      if (!count) p.commandPhase = 0
      else {
        const center = { x: (p.x & 0xfe00) + 256, y: (p.y & 0xfe00) + 256, h: p.h },
          spacing = Math.trunc(2048 / count)
        let a = p.timer
        for (const q of here())
          if (q.flags3 & 16) {
            q.flags3 = (q.flags3 & ~16) >>> 0
            q.flags2 = (q.flags2 | 0x200200) >>> 0
            const target = step(center, a, 128)
            if (
              Math.abs(short(target.x) - short(q.x)) < 12 &&
              Math.abs(short(target.y) - short(q.y)) < 12
            ) {
              q.speed = 0
              face(q, bearing(q, center), e, true)
            } else {
              e.destination(q, target)
              if (!q.speed) q.speed = randomPersonSpeed(w, q)
            }
            a = (a + spacing) & 2047
          }
      ...
      p.commandPhase = byte(p.commandPhase - 1)
      if (p.commandPhase < 1) transition(p, 1)
      break
    }
    case 4: {
      let count = 0,
      ...
      for (const q of here())
        if (same(q)) {
          if (q.substate === 3) leader = q
          else if (q.substate === 4) count++
        }
    ...
    case 5: {
      ...
      if (started) {
        p.animationMode = 0
        p.assignment |= 16
        p.timer = (random(w) & 255) + 256
        p.link = 0
      }
      let recruit = started
      const action = p.animationMode,
        record = rules.celebrationChain[action],
        changed = !!(p.assignment & 16)
      if (changed) {
        p.assignment &= ~16
        p.commandPhase = record[0]
      }
      p.commandPhase = byte(p.commandPhase - 1)
      if (p.commandPhase < 1) {
        p.assignment |= 16
        p.animationMode = record[2]
      }
      ...
      for (let q = w.people.get(p.link); q; q = w.people.get(q.link)) {
        ...
        if (!record[3]) face(q, bearing(q, previous), e, true)
          ...
          q.flags2 = (q.flags2 | 0x200200) >>> 0
          e.destination(q, step(previous, bearing(previous, q), 96))
        }
        previous = q
      }
      if (!valid) p.timer = 0
        ...
        if (recruit) {
          ...
          for (let q: Celebrant | undefined = p; q; q = w.people.get(q.link)) {
            ...
            lastCell = c
            for (const other of w.cellPeople(c))
              if (same(other) && [1, 2, 3, 4].includes(other.substate)) {
                transition(other, 6)
                other.target = p.id
                other.link = 0
              }
    ...
    case 6: {
      ...
      if (leader && (leader.flags2 & 1 || !leader.class || leader.substate !== 5))
        leader = undefined
      if (enter(p)) {
        p.link = 0
        if (leader) {
          if (!leader.link) leader.link = p.id
            ...
            for (let q = w.people.get(leader.link); q && q !== p; q = w.people.get(q.link)) {
              if (!q.link) {
                q.link = p.id
                break
              }
    ...
    case 8:
      ...
      p.timer = short(p.timer - 1)
      if (p.timer < 1) {
        p.timer = (random(w) & 31) + 32
        face(p, random(w) & 2047, e)
      }
      break
  }
}
