...
export type BuildingCells={flags:Uint32Array;buildingIds:Uint16Array;owners:Uint8Array;shadows:Uint8Array};
...
export function registerBuildingFootprint(land:BuildingCells,b:RegisteredBuilding,mode:number,
  ...
  for(const i of buildingFootprintCells(b)){
    ...
    if(mode===0||mode===1)land.shadows[i]=(land.shadows[i]&240)|(Math.min(15,shade(i))&255);
  }
  refresh(cx|(cy<<8),Math.max(s.width>>1,s.height>>1)+1);
}

// 0x450d50: native building stage shade followed by cell-list scenery shade.
export function nativeCellShade(flags:number,b:{class:number;model:number;state:number;flags2:number;stage:number}|undefined,
  objects:Iterable<{class:number;model:number}>){
  let shade=0;
  if((flags&0x200)&&b&&!(b.flags2&1)&&b.class){
    shade=rules.buildingShade[b.model];
    if(b.state===1&&!(rules.buildingFlags[b.model]&256))shade=Math.trunc(shade*((b.stage<<24)>>24)/3);
  }
  for(const o of objects){if(shade>15)break;if(o.class===5)shade+=rules.sceneryShade[o.model];}
