...
export type TooltipObject = Point & {
  id: number; type: number; model: number; owner: number;
  tutorial: number; head: {type: number; flags: number} | null;
};
export type TooltipState = {
  target: number; flags: number; remaining: number; text: string;
  fixed: number; draw: number; hold: number; scroll: number;
};
export const createTooltip = (): TooltipState => ({target:0,flags:0,remaining:0,text:'',fixed:0,draw:0,hold:0,scroll:0});
...
export function tooltipName(object: TooltipObject, player = 0, multiplayer = false) {
  let stringId = 0, tribe = false;
  const names = native.names as Record<number, number[][]>;
  if (object.type === 2 && object.model === 18 || object.type === 5 && object.model === 9) {
    if (object.head) {
      stringId = object.tutorial >= 1 && object.tutorial <= 3 ? 890 + object.tutorial :
        object.head.type === 3 ? 889 : object.head.type === 5 ? 899 :
        object.head.flags & 32 ? 898 : object.head.flags & 16 ? 897 : 888;
    }
    ...
    stringId = record[0];
    if (record[1] && (multiplayer || object.owner !== player)) { stringId = record[1] + Number(multiplayer); tribe = true; }
...
export function showObjectTooltip(state: TooltipState, object: TooltipObject | null, duration: number, player = 0, multiplayer = false) {
  clearTooltip(state);
  if (!object) return;
  ...
  if (name.tribe && object.owner !== -1) state.text = state.text.replace('%s',['Blue','Dakini','Chumara','Matak'][object.owner]);
}
...
export function stepTooltip(state: TooltipState, targetValid: boolean, frameRate: number) {
  ...
  if (state.remaining < 1 || (!(state.flags & 8) && (!targetValid || !(state.flags & 4)))) clearTooltip(state);
  else { state.draw = 1; state.scroll = 0; state.hold = Math.imul(frameRate,2); }
  return true;
}

export function worldTooltipObject(world: World, id: number): TooltipObject | null {
  const b = world.buildings.find(b => b.id === id && b.hp > 0);
  if (b) return {...b,type:2,model:b.kind==='hut'?b.level:b.kind==='camp'?7:b.kind==='tower'?4:5,owner:b.team==='blue'?0:1,tutorial:0,head:null};
  const shrine = world.shrines.find(s => s.id === id);
  if (!shrine) return null;
  const trigger = level.objects.find(o => o.type===6 && o.model===6 && distance(o,shrine)<.01);
  ...
  for (let i=6;i<26;i+=2) {
    const reward = level.objects.find(o => o.index+1===(settings[i]|settings[i+1]<<8) && o.type===6 && o.model===2);
    if (reward?.settings?.[2]===1) flags |= 16;
    if (reward?.settings?.[2]===3) flags |= 32;
  }
  const vault = shrine.kind==='vault' ? level.objects.find(o => o.type===2 && o.model===18 && distance(o,shrine)<3) : null;
  const point = vault ?? shrine;
  return {id,x:point.x,z:point.z,type:vault?2:5,model:vault?18:9,owner:-1,tutorial:0,
...
export function forcedTooltipObject(world: World, mode: number, packed: number) {
  ...
  if (mode===1) {
    const shrine=world.shrines.find(s => s.kind!=='vault' && distance(nativeCellPoint((((-s.z-8)&255)<<8)|((s.x+8)&255)),point)===0);
  ...
  for (const b of [...world.buildings,...world.shrines.filter(s=>s.kind==='vault')]) {
    ...
    if (object && Math.abs(object.x-point.x)<=2 && Math.abs(object.z-point.z)<=2) return object;
  }
  return null;
}
