...
export type AnimatedUnit = Animation & {
  stamp: number
  flags3: number
  morphTimer: number
  morphFrames: number
...
export type AnimationData = {
  ...
  modelFrames: readonly (readonly number[])[]
  morphDurations: ArrayLike<number>
}
export type PersonAnimation = Animation & {
  model: number
  state: number
  flags2: number
  flags3: number
...
export function stepPersonPose(
  p: Animation & { animationMode: number; assignment: number; commandPhase: number },
  ...
  if (p.animationMode === 2) {
    if (p.assignment & 16) {
      ...
      p.f1 = 0
      p.f2 = 0
      p.commandPhase = (random(poseRandom) & 7) + 4
    }
...
export function setAnimationObject(s: Animation, draw: number, object: number) {
  draw &= 255
  const d = rules.animationDescriptors[draw]
  if (!d) throw new RangeError(`Unimported animation descriptor ${draw}`)
  s.object = object & 65535
  s.draw = draw
  s.morph = 0
  s.renderFlags = d.flags | 256
  s.palette = d.palette
...
export function stepObjectAnimation(
  p: AnimatedUnit,
  w: { counter: number; levelFlags: number; levelFlags2: number },
  data: AnimationData,
  footprints: () => void
) {
  ...
  switch (d.mode) {
    ...
    case 2:
      ...
      break
    case 3: {
      const frames = data.modelFrames[p.morph]
      p.f1 = short(p.f1 + d.step)
...
export function stepAnimations(
  lists: readonly (readonly AnimatedUnit[])[],
  landFlags: number,
  w: { counter: number; levelFlags: number; levelFlags2: number },
  data: AnimationData,
  footprints: (p: AnimatedUnit) => void
) {
  if (landFlags & 2) return
  for (const list of lists)
    for (const p of list) stepObjectAnimation(p, w, data, () => footprints(p))
}

// 0x4d4040: the upper-body setter also handles vehicle poses and visibility.
export function setPersonAnimation(
  p: PersonAnimation,
  object: number,
  w: {
    playerTribe: number
    gameFlags: number
    sessionSubstate: number | null
    tribes: { flags: number; playerType: number }[]
    objects: Map<number, { flags2: number; class: number; passenger: number; speed: number }>
  },
  data: Pick<AnimationData, 'frameCounts'>
) {
  const previous = p.renderFlags,
    assignment = p.assignment
  p.assignment &= ~128
  const set = (id: number) => {
    const [start, draw] = rules.animationObjects[id]
    setAnimationObject(p, draw, start)
  }
  if (
    p.flags4 & 0x800000 &&
    p.vehicle &&
    ![11, 14, 41].includes(p.state) &&
    !(assignment & 0x200)
  ) {
    set(rules.personAnimationObjects[24 * 9 + p.model])
    p.f1 = 0
    p.renderFlags |= 2
    p.f2 = 0
    const passengerId = w.objects.get(p.vehicle)!.passenger
    const passenger = passengerId ? w.objects.get(passengerId) : undefined
    if (passenger && !(passenger.flags2 & 1) && passenger.class && passenger.speed)
      p.renderFlags &= ~2
  } else set(object)
  if (p.state === 33 && w.tribes[p.tribe].playerType !== 1 && !(p.flags2 & 0x80000)) {
    set(rules.personAnimationObjects[7 * 9 + p.model])
    p.renderFlags |= 2
    p.f1 = 0
    p.f2 = 1
  }
