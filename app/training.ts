...
export type TrainingWorld = QueueWorld & {
  ...
  buildings: Map<number, TrainingBuilding>
}
...
export function trainingQueuePredecessor(w: QueueWorld, b: TrainingQueue, person: QueuePerson) {
  let p = live(w.people, b.queueHead),
    previous: QueuePerson | undefined
  while (p && p !== person) {
    previous = p
    p = live(w.people, p.reservationNext)
  }
...
export function appendTrainingQueue(w: QueueWorld, b: TrainingQueue, person: QueuePerson) {
  ...
  if (!b.queueHead) b.queueHead = person.id
    ...
    let p = live(w.people, b.queueHead)
    while (p) {
      if (!p.reservationNext) {
        p.reservationNext = person.id
        break
      }
      p = live(w.people, p.reservationNext)
      index++
  ...
  person.flags3 = (person.flags3 | 32) >>> 0
  person.reservationNext = 0
  return index
}
...
export function rebuildTrainingQueue(w: QueueWorld, b: TrainingQueue) {
  ...
  while (p) {
    ...
    const order =
      p.class === 1 && (p.state === 10 || p.state === 14)
        ? currentPersonOrder(w.orders, p)
        : undefined
  ...
  b.activity |= 0x2000
  b.queueFrom = Math.min(Math.max(firstRemoved, 0), 255)
  return count
}
...
export function stepTrainingPerson(w: TrainingWorld, p: TrainingPerson, effects: TrainingEffects) {
  ...
  if (p.substate === 0) {
    ...
    if (
      b &&
      rules.buildingFlags[b.model] & 64 &&
      !(rules.personCommands[6].people & (1 << (p.model & 31)))
    )
      target = 0
    b = live(w.buildings, target)
  }
  ...
  const near = (distance: number) =>
    Math.abs(short(p.goalX) - short(p.x)) < distance &&
    Math.abs(short(p.goalY) - short(p.y)) < distance
  const move = (person: TrainingPerson, point: { x: number; y: number }) =>
    effects.setDestination(person, short(point.x), short(point.y))
  ...
  if (p.substate === 0) {
    ...
    if (!(p.flags3 & 32)) {
      ...
      if (
        effects.adjacentBuilding(p) === b.id &&
        (!queued || (!b.queueHead && byte(b.inside) < capacity && byte(b.entering) < capacity))
      )
        p.substate = 4
      else {
      ...
      p.substate = 3
      p.commandPhase = 0
      let follower = live(w.people, b.queueHead),
        index = 0
      while (follower && follower !== p) {
        follower = live(w.people, follower.reservationNext)
        index = (index + 1) & 255
      }
      if (follower) {
        p.commandPhase = index
        if (near(12)) stopPersonMovement(p, effects.setAnimation)
      } else p.commandAux = 1
    ...
    case 1: {
      ...
      if (!(p.counter & 1)) {
        if (
          queued &&
          near(0x538) &&
          (b.queueHead || byte(b.inside) >= capacity || byte(b.entering) >= capacity)
        )
          p.substate = 2
        else if (near(112)) {
          p.substate = 4
      ...
      if (byte(b.inside) < capacity && !b.entryDelay && p.id === short(b.queueHead) && !p.speed) {
        ...
        if (near(12)) {
          if (p.speed) {
            ...
            const angle = face(
              p.commandPhase ? position(byte(p.commandPhase) - 1) : buildingInsidePoint(b)
            )
          ...
          if (
            ...
            p.reservationNext = next.reservationNext
            next.reservationNext = p.id
            move(p, position(byte(next.commandPhase)))
            recover()
            move(next, position(byte(p.commandPhase)))
            recover(next)
            p.commandAux = 1
            next.commandAux = 1
          }
        ...
        if (p.substate === 4) {
          ...
          if (byte(b.entering) < capacity) {
            if (!(b.flags3 & 64)) {
              ...
              if (byte(b.entering) > rules.buildingCapacity[b.model])
                b.flags3 = (b.flags3 | 64) >>> 0
            }
          } else blocked = true
        }
        if (!blocked) {
