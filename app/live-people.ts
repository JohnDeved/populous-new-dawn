...
function collisionWorld(w:World):CollisionWorld{
  ...
  return {objects,walkMask:w.land.walkMasks[0],boatAt:()=>false,cell:pos=>{
    ...
    return {flags:w.land.flags[i],category:w.land.categories[i],building:w.land.buildingIds[i]};
  }};
}
...
export function stepLiveCelebration(w:World,u:Unit){
  ...
  if(p.speed&&!(p.flags2&0x84000)){
    ...
    const building=collision.cell(p).flags&0x200?w.buildings.find(b=>b.id===(collision.cell(p).building&1023)):undefined;
