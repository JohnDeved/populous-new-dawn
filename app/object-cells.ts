export type CellObject={id:number;x:number;y:number;h:number;flags2:number;flags3:number;
  cellNext:number;cellPrevious:number;displacement:{x:number;y:number;h:number}};
...
export function insertObjectIntoCell(w:ObjectCells,p:CellObject,to:{x:number;y:number}){
  const i=index(to);p.cellPrevious=0;p.cellNext=w.heads[i];
  if(p.cellNext)w.objects.get(p.cellNext)!.cellPrevious=p.id;
  w.heads[i]=p.id;p.flags2=(p.flags2|0x20000)>>>0;
}
...
export function moveObjectInCells(w:ObjectCells,p:CellObject,to:{x:number;y:number;h:number}){
  const old={x:p.x,y:p.y,h:p.h},moved=index(p)!==index(to);
  if(moved){removeObjectFromCell(w,p);insertObjectIntoCell(w,p,to);}
  p.x=to.x&65535;p.y=to.y&65535;p.h=short(to.h);
  if((p.flags3&0x100)&&!(p.flags3&0x200)){
    p.displacement.x=short(p.x-old.x);p.displacement.y=short(p.y-old.y);p.displacement.h=short(p.h-old.h);
  }
  return moved;
}
...
export function* objectsInCell(w:ObjectCells,cell:number){
  ...
  while(id){const p=w.objects.get(id)!;yield p;id=p.cellNext;}
}
