...
export function terrainTile(
  ...
  const starts = fields.map(v => v[0] << 16),
    dx = fields.map(v => (v[1] - v[0]) << 11),
    dy = fields.map(v => (v[3] - v[0]) << 11),
    cross = fields.map(v => (v[2] - v[3] - v[1] + v[0]) << 6)
  ...
  for (let y = 0; y < 32; y++) {
    ...
    for (let x = 0; x < 32; x++) {
      const d = t.detail[((oy + y) & 255) * 256 + ((ox + x) & 255)],
        next = t.detail[((oy + y + 1) & 255) * 256 + ((ox + x + 1) & 255)]
      const shade = Math.max(0, Math.min(255, (row[0] >> 16) + ((next - d) >> 2))),
        level = row[1] >> 16
...
export function terrainAtlas(
  ...
  const brightness = new Uint8Array(16384),
    ...
    out = previous?.pixels ?? new Uint8Array(4096 * 4096 * 4)
  ...
  for (let z = 0; z < 128; z++)
    for (let x = 0; x < 128; x++) {
      ...
      if (
        ...
        continue
      const tile = terrainTile(land, brightness, cell, t)
      updated++
      for (let y = 0; y < 32; y++)
        for (let px = 0; px < 32; px++) {
          const at = ((z * 32 + 31 - y) * 4096 + x * 32 + px) * 4,
            color = tile[y * 32 + px] * 4
