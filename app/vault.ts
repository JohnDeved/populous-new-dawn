...
export type VaultAction = 'face' | 'pray' | 'open' | 'enter' | 'trigger' | 'exit' | 'close' | 'leave';
...
export function stepVaultTask(task: VaultTask, arrived: boolean, ready: boolean, triggerExists: boolean) {
  const actions: VaultAction[] = [];
  const next = (phase: number) => { task.phase = phase; task.entering = true; };
  ...
  switch (task.phase) {
    case 2:
      ...
      actions.push('pray');
      if (!arrived) next(1); // Native task returns to its approach state.
      else if (ready) next(3);
      break;
    case 3:
      if (first) { task.remaining = 40; actions.push('open'); }
    ...
    case 5:
      ...
      if (--task.remaining < 1) { actions.push('trigger'); next(6); }
      break;
    case 6:
      if (first) task.remaining = 24;
    ...
    case 7:
      ...
      break;
    case 8:
      if (first) { task.remaining = 40; actions.push('face', 'close'); }
