...
export type DefeatEntity = {id:number;class:number;model:number;tribe:number;flags4:number;hp:number;
  buildingFlags:number;damage:number;internalModel:number};
...
export function defeatTribe<T extends DefeatEntity>(w:{turn:number;lastDefeated:number;skyCounter:number;units:T[]},
  tribe:number,flags:number,position:{x:number;y:number;h:number},effects:{
    allocate:(tribe:number,record:number[])=>void;reveal:(position:{x:number;y:number;h:number})=>void;remove:(p:T)=>void;
  }) {
  tribe=byte(tribe);w.lastDefeated=tribe;
  if((w.turn>>>0)>32)w.skyCounter=20;
  if(!(flags&0x10000)){effects.allocate(tribe,[tribe,0,0,0,0]);effects.reveal(position);}
  for(const p of [...w.units])if(p.tribe===tribe) {
    if(p.class===1) {
      if(p.flags4&0x800)effects.remove(p);
      else if(p.model===8)p.hp=2;
    }else if(p.class===2) {
      p.buildingFlags|=64;p.damage=short(rules.buildingDamageThreshold[p.model]-(p.id&15)*16);
    }else if(p.class===10&&p.model===12&&p.internalModel===7)effects.remove(p);
  }
}

export type DamageBuilding = {model:number;state:number;flags2:number;flags3:number;buildingFlags:number;
...
export function changeBuildingWork(plan:BuildingPlan,amount:number,b:DamageBuilding|null,overlay:{stage:number}|null,
  effects:{move:(b:DamageBuilding)=>void;release:(b:DamageBuilding)=>void;init:(b:DamageBuilding)=>void}) {
  ...
  if(amount>0){plan.attacker=255;b.attacker=255;}
  return changed;
}
...
export function processBuildingDamage(w:{randomState:number;tribes:{playerType:number;people:number[]}[]},b:DamageBuilding,effects:{
  ensurePlan:()=>void;plan:()=>BuildingPlan|null;changeWork:(plan:BuildingPlan,amount:number)=>boolean;
  removeOccupant:()=>void;smoke:()=>{duration:number}|null;debris:(oldStage:number)=>void;
  canRespond:(person:number)=>boolean;reserve:(person:number)=>void;removePlan:(plan:BuildingPlan)=>void;
  removeBuilding:()=>void;notify:()=>void;sound:()=>void;
}) {
  if(b.flags3&128)return;
  const model=b.model,threshold=rules.buildingDamageThreshold[model];
  if(!(b.counter&3)&&short(b.damage))b.damage=short(b.damage-1);
  if((b.damage&65535)<threshold)return;
  const oldStage=byte(b.stage);effects.ensurePlan();const plan=effects.plan();
  if(!plan)return;
  ...
  for(const t of w.tribes)if(t.playerType===1)for(const person of t.people)if(effects.canRespond(person)){effects.reserve(person);break;}
  b.damage=short(b.damage-threshold);
  if(short(plan.remaining)<1){effects.removePlan(plan);effects.notify();effects.removeBuilding();}
