...
export type PersonOrder = {model: number; flags: number; references: number; object: number; a: number; b: number};
...
export type OrderedPerson = {
  id: number; model: number; state: number; substate: number; x: number; y: number;
  flags2: number; flags3: number; flags4: number; assignment: number; selectionFlags: number;
  commands: number[]; commandCursor: number; immediateCommand: number;
  orderLocation: number; commandStatus: number; workTarget: number;
};
...
export function writePersonOrder(order: PersonOrder, model: number, first: number, point: number, flags: number) {
  model &= 255; first &= 65535; point &= 65535;
  const type = descriptor(model).flags;
  order.model = model; order.flags = flags & 254;
  if (type & 1) {
    order.a = ((point << 8) + 128) & 65535;
    order.b = (point & 0xff00) + 128;
  } else if (type & 0x242) order.a = first;
  else if (type & 4) { order.a = first; order.b = point; }
  else if (type & 0x800) { order.a = point; order.b = ((first << 8) | (first >>> 8)) & 65535; }
...
export function allocatePersonOrder(pool: OrderPool) {
  ...
  for (let scanned = 0; scanned < 800; scanned++, index++) {
    ...
    order.model = 0; order.flags = 0; order.object = 0;
...
export function normalizePersonRoute(pool: OrderPool, person: OrderedPerson) {
  ...
  for (let slot = first; slot <= last; slot++) {
    ...
    if (person.commands[slot] && order.model === 11) {
      order.model = 25;
      const cell = order.a;
      order.a = (cell & 254) << 8; order.b = cell & 0xfe00;
    }
  }
...
export function removePersonOrder(pool: OrderPool, person: OrderedPerson, slot: number, effects: OrderEffects) {
  ...
  if (id) {
    if (person.state === 10 || person.state === 33) {
      ...
      if (command && !(command.flags & 1) && [19, 21].includes(command.model) && person.substate === 1) effects.stopWork(person);
    }
  ...
  if (person.assignment & 32) { person.assignment &= ~32; effects.releaseFight(person); }
}

// 0x436d00, including immediate replacement after the new reference is acquired.
export function attachPersonOrder(pool: OrderPool, person: OrderedPerson, id: number, slot: number, effects: OrderEffects) {
  const order = pool.records[id];
  if (!order.references) pool.active = (pool.active + 1) & 65535;
  order.references = (order.references + 1) & 65535;
  if (slot >= 0) {
    person.flags3 = (person.flags3 & ~0x2000000) >>> 0; person.orderLocation = 0;
    ...
    if (!(person.flags3 & 0x2000000)) person.orderLocation = (((person.x >>> 8) & 254) | (person.y & 0xfe00)) + 1;
  }
}
...
export function commitPersonOrders(pool: OrderPool, group: OrderGroup, people: OrderedPerson[],
  models: [number, number, number], effects: OrderEffects) {
  const ids: number[] = [];
  let success = true;
  for (let i = 0; i < group.count; i++) {
    const id = allocatePersonOrder(pool); ids.push(id);
  ...
  if (success) for (const person of people) {
    ...
    for (let slot = 0; slot < group.count; slot++, position = (position + 1) % 8) {
      ...
      const eligible = person.flags4 & 0x800 ? d.flags & 0x4000000 : d.people & (1 << (person.model & 31));
