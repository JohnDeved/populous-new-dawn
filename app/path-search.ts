...
const category = (w: PathSearchWorld, p: Uint8Array) =>
  rules.terrainCategoryFlags[w.categories[((p[1] & 254) >> 1) * 128 + ((p[0] & 254) >> 1)] & 15]
...
export function pathBoatInCell(
  w: Pick<PathSearchWorld, 'boatsEnabled' | 'cellObjects'>,
  cell: number
) {
  if (w.boatsEnabled)
    for (const p of w.cellObjects(cell))
      if (p.class === 4 && !(rules.vehicleRestFlags[p.model] & 1)) return p.id
  return 0
}
...
export function compactPathPoints(path: SearchPath) {
  ...
  for (let i = 0; i < path.count; i++) {
    ...
    if (
      i + 1 < path.count &&
      v.getInt32(a, true) === v.getInt32(b, true) &&
      v.getInt32(a + 4, true) === v.getInt32(b + 4, true)
    )
      continue
    d.copyWithin(20 + output * 10, a, a + 10)
    output++
  }
  path.count = output
}
...
export function collectSearchPath(
  path: SearchPath,
  result: Uint8Array,
  queuedCommandModel: number
) {
  const d = path.data,
    v = new DataView(d.buffer, d.byteOffset, d.byteLength),
    abs = (n: number) => Math.abs(n | 0) | 0
  ...
  emit(0, v.getInt32(0, true), v.getInt32(4, true), d[8])
  emit(4, v.getInt32(10, true), v.getInt32(14, true), d[18])
  result[1032] = path.count
  let previous = 0,
    output = 8
  for (let i = 0; i < path.count; i++) {
    const at = 20 + i * 10,
      x = v.getInt32(at, true),
      y = v.getInt32(at + 4, true),
      px = v.getInt32(previous, true),
      py = v.getInt32(previous + 4, true)
    if (abs(x - px) >= 128 || abs(y - py) >= 128) {
      emit(output, (x + px) >> 1, (y + py) >> 1, d[at + 8])
      output += 4
      result[1032]++
    }
    emit(output, x, y, d[at + 8])
    output += 4
    previous = at
  }
  const count = result[1032],
    last = 4 + count * 4,
    previousPoint = count * 4
  if (
    count > 1 &&
    rules.personCommands[queuedCommandModel].flags & 0x4000 &&
    result[4] === result[last] &&
    result[5] === result[last + 1]
  ) {
    const near = (at: number) =>
      Math.abs(result[last] - result[at]) < 3 && Math.abs(result[last + 1] - result[at + 1]) < 3
    if (!result[last + 2] && result[previousPoint + 2] === 1) {
      if (near(previousPoint)) {
        result[1032]--
        return true
      }
    } else if (
      count > 2 &&
      !result[last + 2] &&
      !result[previousPoint + 2] &&
      result[previousPoint - 2] === 1 &&
      near(previousPoint - 4)
    ) {
      result[1032] -= 2
      return true
    }
...
export function searchPersonPath(
  w: PathSearchWorld,
  p: SearchPerson,
  from: Uint8Array,
  to: Uint8Array,
  ...
  from[2] = boat ? 1 : 0
  if (boat) s.currentBoat = boat
  to[2] = category(w, to) & 1 ? 0 : 1
  w.result[1032] = 0
  w.result.set(from.subarray(0, 4), 0)
  w.result.set(to.subarray(0, 4), 4)
  if (from[0] === to[0] && from[1] === to[1]) return finish(0)
  const aboard = !!p.vehicle && !(p.flags4 & 0x2000000),
    allow = aboard || vehicles
  if (!aboard && to[2] && category(w, to) & 2) return finish(1)
  let first = !w.boatsEnabled || !allow || category(w, to) & 1 ? 1 : 0,
    last = 2
  if (option && w.boatsEnabled && allow) {
    first = 4
    last = 5
  }
  s.searches = (s.searches + 1) >>> 0
  s.landLimit = w.landLimit & 65535
  s.checkingPerson = p.id
  s.limit = (w.tribes[p.tribe].playerType === 1 ? w.computerLimit : w.humanLimit) & 255
  s.vehicles = Number(allow)
  e.prepare()
  s.candidateIndex = 0
  let found = false,
    secondary = false
  while (s.candidateIndex < s.candidateCount) {
    ...
    for (let mode = first; mode < last; mode++) {
      ...
      for (const mask of [1, 0]) {
        ...
        else if (result === 2) secondary = true
        if (found) break
      }
