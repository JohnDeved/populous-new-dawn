...
import { BUILDINGS, SPELLS, HOME, createWorld, select, maxHp, tell, type World, type UnitKind } from './model';
...
export default function Home() {
  ...
  const huts = world.buildings.filter(b => b.team === 'blue' && b.kind === 'hut' && b.progress === 1).length;
  const manaRate = .6 + blue.length * .055 + world.buildings.filter(b => b.team === 'blue' && b.kind === 'temple' && b.progress === 1).length * 1.2;
  ...
  const objectives = [ { text: 'Welcome the wildmen', done: world.stats.converted > 0 }, { text: 'Expand your settlement', done: world.stats.built > 0 }, { text: 'Defeat the Dakini tribe', done: world.status === 'won' } ];
  return <main className="game-shell">
    ...
    <header className="topbar">
      ...
      <div className="resource-strip">
        <div className="resource" title="Your followers / housing capacity"><span className="resource-icon people-icon">♟</span><span><strong>{blue.length}<em> / {huts * 6 + 1}</em></strong><small>FOLLOWERS</small></span></div>
        <div className="resource" title="Idle braves gather wood automatically"><span className="resource-icon wood-icon">⫻</span><span><strong>{Math.floor(world.wood)}</strong><small>WOOD</small></span></div>
        <div className="resource mana-resource" title="Followers and temples generate mana"><span className="resource-icon mana-icon">✧</span><span><strong>{Math.floor(world.mana)}<em className="rate">+{manaRate.toFixed(1)}/s</em></strong><small>MANA</small></span><div className="tiny-mana"><i style={{ width: `${world.mana / 3}%` }} /></div></div>
      </div>
    ...
    <div className="chapter-heading"><span className="eyebrow"><i /> WORLD 01 <b> / </b> A NEW BEGINNING</span><h1>The First Dawn</h1><p>From a handful of followers, a civilisation rises.</p><div className="chapter-rule"><span />◆<span /></div></div>

    <aside className="left-hud">
      <section className="objective-panel"><div className="panel-eyebrow"><span>YOUR DESTINY</span><span>01 — 03</span></div><h2>One world. One tribe.</h2><p>Guide your people. Conquer the Dakini.</p><ol>{objectives.map((o, i) => <li key={o.text} className={o.done ? 'complete' : ''}><span>{o.done ? '✓' : `0${i + 1}`}</span>{o.text}</li>)}</ol></section>
      <section className="tribe-panel"><div className="tribe-heading"><span className="tribe-dot blue-dot" />YOUR TRIBE<span className="live-label">THRIVING</span></div><div className="tribe-classes">{([{ kind: 'brave', label: 'Braves', icon: '♟' }, { kind: 'warrior', label: 'Warriors', icon: '⚔' }, { kind: 'shaman', label: 'Shaman', icon: '✧' }] as const).map(u => <button key={u.kind} title={`Select all ${u.label.toLowerCase()}`} onClick={() => choose(u.kind)} className={selected.length > 0 && selected.every(s => s.kind === u.kind) ? 'selected' : ''}><span>{u.icon}</span><strong>{blue.filter(b => b.kind === u.kind).length}</strong><small>{u.label}</small></button>)}</div><button className="select-all" onClick={() => choose('all')}>Select all followers <span>↗</span></button></section>
    ...
    <div className="bottom-left">
      {hints && <div className="hint"><span className="hint-icon">✧</span><div><strong>A little faith goes a long way.</strong><p>Choose Convert, then click the pale wildmen near the centre of the island.</p></div><button aria-label="Dismiss tip" onClick={() => setHints(false)}>×</button></div>}
      <section className="selection-panel"><button className={`portrait ${isShaman ? 'shaman-portrait' : 'follower-portrait'}`} onClick={() => { choose('shaman'); engine.current?.focus(shaman ?? HOME); }} aria-label="Select and focus shaman"><span className="portrait-halo" /><span className="portrait-body" /><span className="portrait-face" /><span className="portrait-crown">♆</span><span className="portrait-staff" /></button><div className="selected-info"><div className="panel-eyebrow">{isShaman ? 'THE CHOSEN ONE' : selected.length ? 'YOUR FOLLOWERS' : 'YOUR SHAMAN'}</div><h3>{isShaman ? 'Shaman' : selected.length ? `${selected.length} selected` : world.respawn ? 'Reincarnating' : 'Awaiting orders'}</h3><div className="health-bar"><i style={{ width: `${isShaman ? selected[0].hp : shaman?.hp ?? 0}%` }} /></div><div className="selection-meta"><span>{isShaman ? `${Math.ceil(selected[0].hp)} / 100 health` : world.respawn ? `${Math.ceil(world.respawn)}s until rebirth` : 'Click the world to move'}</span><button onClick={() => engine.current?.focus(selected[0] ?? shaman ?? HOME)} title="Focus selection · F" aria-label="Focus selection">⌖</button></div></div></section>
    </div>

    <section className="command-dock" aria-label="Command panel">
      <nav className="dock-tabs" aria-label="Command categories">{(['spells', 'buildings', 'followers'] as const).map(t => <button key={t} onClick={() => { setTab(t); world.mode = null; setHover(null); update(); }} className={tab === t ? 'active' : ''}><span>{t === 'spells' ? '✧' : t === 'buildings' ? '⌂' : '♟'}</span>{t}<small>{t === 'spells' ? '1–6' : t === 'buildings' ? 'B' : ''}</small></button>)}</nav>
      <div className="dock-content">
        {tab === 'spells' && <div className="spell-list">{SPELLS.map(s => <button key={s.id} className={`spell-card ${world.mode === s.id ? 'active' : ''} ${world.mana < s.cost ? 'unaffordable' : ''}`} style={{ '--spell-color': s.color } as CSSProperties} onClick={() => { world.mode = world.mode === s.id ? null : s.id; update(); }} onMouseEnter={() => setHover(s.id)} onMouseLeave={() => setHover(null)} onFocus={() => setHover(s.id)} onBlur={() => setHover(null)} aria-label={`${s.name}, ${s.cost} mana`} aria-pressed={world.mode === s.id}><span className={`spell-art spell-${s.id}`}><kbd>{s.key}</kbd><span className="spell-glyph">{s.symbol}</span><span className="spell-orbit" /></span><strong>{s.name}</strong><small><span>✧</span> {s.cost}</small></button>)}</div>}
        {tab === 'buildings' && <div className="building-list">{BUILDINGS.map(b => <button key={b.id} className={`building-card ${world.mode === b.id ? 'active' : ''} ${world.wood < b.cost ? 'unaffordable' : ''}`} onClick={() => { world.mode = world.mode === b.id ? null : b.id; update(); }} onMouseEnter={() => setHover(b.id)} onMouseLeave={() => setHover(null)} onFocus={() => setHover(b.id)} onBlur={() => setHover(null)} aria-label={`${b.name}, ${b.cost} wood`} aria-pressed={world.mode === b.id}><span>{b.symbol}</span><strong>{b.name}</strong><small>⫻ {b.cost} wood</small></button>)}</div>}
        {tab === 'followers' && <div className="follower-list">{([{ id: 'shaman', label: 'Shaman', description: 'Lead & cast spells', icon: '✧' }, { id: 'brave', label: 'Braves', description: 'Gather & construct', icon: '♟' }, { id: 'warrior', label: 'Warriors', description: 'Defend & conquer', icon: '⚔' }, { id: 'all', label: 'Everyone', description: 'Command your tribe', icon: '♟♟' }] as const).map(u => <button key={u.id} onClick={() => choose(u.id)}><span>{u.icon}</span><strong>{u.label}</strong><small>{u.description}</small></button>)}</div>}
      </div>
      <div className="dock-caption"><span className="caption-spark">✦</span><span>{focused?.description ?? (tab === 'spells' ? 'Your followers give you faith. Your faith gives you power.' : tab === 'buildings' ? 'Place a building on open land. Free braves will construct it.' : 'Select your people, then click a destination or enemy.')}</span></div>
    </section>
    ...
    <div className="camera-tools"><button onClick={() => engine.current?.zoom(.82)} aria-label="Zoom in">+</button><button onClick={() => engine.current?.zoom(1.22)} aria-label="Zoom out">−</button><button onClick={() => engine.current?.focus(HOME)} aria-label="Centre camera">⌖</button></div>
    <footer className="controls-footer"><span><kbd>CLICK</kbd> Select / move</span><span><kbd>DRAG</kbd> Select group</span><span><kbd>RIGHT DRAG</kbd> Orbit</span><span><kbd>W A S D</kbd> Pan</span><span><kbd>SCROLL</kbd> Zoom</span><button onClick={() => { world.speed = world.speed === 1 ? 2 : 1; update(); }} title="Change game speed">{world.speed}× <span>GAME SPEED</span></button><span className="tribute-label">A BROWSER TRIBUTE</span></footer>
    ...
    <dialog ref={dialog} className="game-dialog" onClose={() => { setMenu(false); world.paused = false; update(); }}><button className="dialog-close" onClick={() => setMenu(false)} aria-label="Close menu">×</button><span className="eyebrow">POPULOUS · THE FIRST DAWN</span><h2>The world can wait.</h2><p>Lead your tribe, shape the land, and defeat every Dakini follower and building.</p><div className="menu-actions"><button className="primary-button" onClick={() => setMenu(false)}>Return to the world <span>↗</span></button><button className="secondary-button" onClick={restart}>Restart world</button></div><h3>Your powers, at a glance</h3><div className="help-grid"><span>Click / tap</span><strong>Select a follower or give a move / attack order</strong><span>Drag on land</span><strong>Select a group · Shift adds to selection</strong><span>Right drag / one-finger drag</span><strong>Orbit the world</strong><span>Scroll / pinch</span><strong>Zoom · Middle drag or WASD to pan</strong><span>1–6 / B</span><strong>Choose a spell / open buildings</strong><span>H / F / Space / Esc</span><strong>Shaman / focus / pause / cancel spell</strong></div><p className="help-tip">Spells reach 34 paces from your shaman and can hurt your own followers. Huts grow the tribe; idle braves gather wood. Build a warrior camp near your braves to train them, and temples to generate more mana.</p><div className="audio-settings"><button className="secondary-button" onClick={toggleSound}>{sound ? '♪ Sound on' : '♫ Enable sound'}</button><label>Volume <input type="range" min="0" max="1" step=".05" value={volume} onChange={e => { const v = Number(e.target.value); setVolume(v); audio.current?.setVolume(v); }} /></label></div><details className="reference-details"><summary>About this recreation & references</summary><p>A single-world fan tribute with original procedural models and synthesised audio. Inspired by Bullfrog’s 1998 game. This is a playable reinterpretation, with simplified economy and AI.</p><div><a href="https://store.steampowered.com/app/2616430/Populous_The_Beginning/" target="_blank" rel="noreferrer">Original game & visual references ↗</a><a href="https://wiki.popre.net/Populous%3A_The_Beginning" target="_blank" rel="noreferrer">Gameplay reference ↗</a><a href="https://www.youtube.com/watch?v=kIY_4F47R6Q" target="_blank" rel="noreferrer">Mark Knight’s original soundtrack ↗</a><a href="https://sounds.spriters-resource.com/pc_computer/populousbegin/asset/393974/" target="_blank" rel="noreferrer">Shaman voice reference ↗</a></div></details></dialog>
  </main>;
}
