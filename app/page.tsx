...
import { BUILDINGS, SPELLS, HOME, select, tell, manaRate, populationLimit, guardShaman, type UnitKind } from './model';
...
export default function Home() {
  ...
  const beds=populationLimit(world,'blue');
  ...
  return <main className="game-shell" onClickCapture={e => {
    ...
    <header className="topbar">
      <button className="wordmark" onClick={() => setMenu(true)} aria-label="Populous game menu"><span className="brand-rune">⟡</span><span>POPULOUS<small>THE BEGINNING</small></span></button>
      <div className="resource-strip">
        <div className="resource" title="Your followers / population supported by your settlement"><span className="resource-icon people-icon">♟</span><span><strong>{blue.length}<em> / {beds}</em></strong><small>FOLLOWERS</small></span></div>
      ...
      <div className="top-actions"><span className="clock-label">{timeLabel(world.time)}</span><button className={`icon-button sound-button ${sound ? 'on' : ''}`} onClick={toggleSound} title={sound ? 'Mute sound' : 'Enable sound'} aria-label={sound ? 'Mute sound' : 'Enable sound'}><span>{sound ? '♪' : '♫'}</span>{!sound && <i />}</button><button className="icon-button pause-button" onClick={() => { store.change(w => { w.paused = !w.paused; }); }} aria-label={world.paused ? 'Resume game' : 'Pause game'} title="Pause / resume · Space">{world.paused ? '▷' : 'Ⅱ'}</button><button className="menu-button" aria-label="Menu" onClick={() => setMenu(true)}><span>☰</span><span>Menu</span></button></div>
    </header>
    ...
    <footer className="controls-footer"><span><kbd>CLICK</kbd> Select / move</span><span><kbd>DRAG</kbd> Select group</span><span><kbd>RIGHT DRAG</kbd> Turn</span><span><kbd>W A S D</kbd> Move</span><span><kbd>SCROLL</kbd> Zoom</span><button onClick={() => { store.change(w => { w.speed = w.speed === 1 ? 2 : 1; }); }} title="Change game speed">{world.speed}× <span>GAME SPEED</span></button><span className="tribute-label">A BROWSER TRIBUTE</span></footer>
