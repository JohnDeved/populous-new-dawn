...
import {cameraConfig,cameraConfigIndex,cameraMatrix,modelMatrix,modelPoint,relativeCoordinate,projectPoint,circularMeshBounds,polygonMeshBounds,type Projection} from './projection.ts';
...
export class RenderView {
  ...
  projection:Projection={...this.config,matrix:cameraMatrix(0,this.config.pitch),centerX:320,centerY:256,fractionX:4,fractionY:4,pixelScaleX:.0625,pixelScaleY:.0625};
  center={x:0,y:0};
  rawCenter={x:0,y:0};
  overview=false;
  bounds=circularMeshBounds(50);
  boundsTexture=new THREE.DataTexture(new Float32Array(444),222,1,THREE.RGFormat,THREE.FloatType);
  uniforms={
    nativeBasis:{value:new Int32Array(9)},nativeSettings:{value:new Int32Array(4)},nativeScreen:{value:new Int32Array(4)},
    nativeCenter:{value:new Int32Array(2)},nativeRawCenter:{value:new Int32Array(2)},nativeMode:{value:1},nativeBounds:{value:this.boundsTexture},nativeCellFraction:{value:new THREE.Vector2()},
  };
  update(width:number,height:number,point:{x:number;z:number},heading:number,zoom:number,overview:boolean,screenWidth=width){
    width=Math.max(1,Math.round(width));height=Math.max(1,Math.round(height));
    this.overview=overview;this.config=cameraConfig(cameraConfigIndex(Math.round(screenWidth),height),Math.round(zoom));
    ...
    const angle=Math.round(heading*1024/Math.PI)&2047,matrix=cameraMatrix(angle,this.config.pitch);
    this.projection={...this.projection,...this.config,width,height,matrix,centerX:(width>>1)+this.config.offsetX,centerY:(height>>1)+this.config.offsetY};
    this.bounds=this.config.boundsMode===1?polygonMeshBounds(this.config.bounds,angle):circularMeshBounds(this.config.diameter);
    const pixels=this.boundsTexture.image.data as Float32Array;for(let i=0;i<222;i++)pixels.set(this.bounds[i],i*2);this.boundsTexture.needsUpdate=true;
    this.uniforms.nativeBasis.value.set(matrix);
    this.uniforms.nativeSettings.value.set([this.config.curvature,this.config.depth,this.config.perspective,this.config.scale]);
    this.uniforms.nativeScreen.value.set([width,height,this.projection.centerX,this.projection.centerY]);
    this.uniforms.nativeCenter.value.set([this.center.x,this.center.y]);this.uniforms.nativeMode.value=overview?0:1;
    this.uniforms.nativeRawCenter.value.set([this.rawCenter.x,this.rawCenter.y]);
    this.uniforms.nativeCellFraction.value.set((this.center.x&510)>>1,(this.center.y&510)>>1);
  }
  relative(p:{x:number;z:number},height:number,unwrapped=false){
    const x=Math.round((p.x+8)*256),y=Math.round((-p.z-8)*256);
    return {x:unwrapped?(x-this.rawCenter.x)>>1:relativeCoordinate(x,this.center.x),y:Math.round(height*45),z:unwrapped?(y-this.rawCenter.y)>>1:relativeCoordinate(y,this.center.y)};
  }
  visible(p:{x:number;z:number},unwrapped=false){
    const q=this.relative(p,0,unwrapped),x=(q.x+((this.center.x&510)>>1))/256+110,row=Math.floor((q.z+((this.center.y&510)>>1))/256)+110;
    const span=this.bounds[row];return !!span&&span[0]>0&&x>=span[0]&&x<=span[1];
  }
  project(p:{x:number;z:number},height:number){return projectPoint(this.relative(p,height),this.projection);}
  screen(world:THREE.Vector3,camera:THREE.Camera,unwrapped=false){
    if(!this.overview){const p=projectPoint(this.relative(world,world.y*128/45,unwrapped),this.projection);return new THREE.Vector4(p.screenX*2/this.projection.width-1,1-p.screenY*2/this.projection.height,(p.z+this.config.depth)/16384-1,1);}
    const a=world.x*Math.PI/128,b=world.z*Math.PI/256,r=70+world.y*128/45;
    const v=new THREE.Vector4(Math.sin(a)*Math.cos(b)*r,Math.cos(a)*Math.cos(b)*r-70,Math.sin(b)*r,1).applyMatrix4(camera.matrixWorldInverse).applyMatrix4(camera.projectionMatrix);
  ...
  pick(mouse:THREE.Vector2,objects:THREE.Object3D[],camera:THREE.Camera){
    let best:{point:{x:number;z:number};object:THREE.Object3D;depth:number}|null=null;
    for(const root of objects)root.traverseVisible(object=>{
      ...
      if(object instanceof THREE.InstancedMesh){
        ...
        const local=new THREE.Matrix4();object.getMatrixAt(instance,local);transform.multiply(local);
        if(!this.overview){
          if(!geometry.boundingBox)geometry.computeBoundingBox();
          const box=geometry.boundingBox!.clone().applyMatrix4(transform),x=this.rawCenter.x/256-8,z=-this.rawCenter.y/256-8;
          if(box.max.x<x-128||box.min.x>x+128||box.max.z<z-128||box.min.z>z+128)continue;
        }
      }
      const world:THREE.Vector3[]=[],screen:THREE.Vector4[]=[],scale=object.userData.nativeScale;
      const origin=new THREE.Vector3().setFromMatrixPosition(transform),basis=modelMatrix(object.parent?.userData.nativeHeading??0),unwrapped=!!object.userData.nativeRelative;
      for(let i=0;i<position.count;i++){
        let p=new THREE.Vector3().fromBufferAttribute(position,i);
        if(scale){
          const raw=[Math.round(p.x*scale*3),Math.round(p.y*scale*3),Math.round(-p.z*scale*3)];
          const q=modelPoint(raw,scale,basis,{x:0,y:0,z:0});p=new THREE.Vector3(q.x,q.y,-q.z).multiplyScalar(1/128).add(origin);
        }else p.applyMatrix4(transform);
        world.push(p);screen.push(this.screen(p,camera,unwrapped));
      }
      const count=index?index.count:position.count;
      for(let i=0;i<count;i+=3){
        const ids=index?[index.getX(i),index.getX(i+1),index.getX(i+2)]:[i,i+1,i+2];
        const [a,b,c]=ids.map(i=>screen[i]);
        if([a,b,c].every(p=>p.z< -1)||[a,b,c].every(p=>p.z>1)||[a,b,c].some(p=>p.w<=0))continue;
        const determinant=(b.y-c.y)*(a.x-c.x)+(c.x-b.x)*(a.y-c.y);if(Math.abs(determinant)<1e-12)continue;
        const u=((b.y-c.y)*(mouse.x-c.x)+(c.x-b.x)*(mouse.y-c.y))/determinant;
        const v=((c.y-a.y)*(mouse.x-c.x)+(a.x-c.x)*(mouse.y-c.y))/determinant,t=1-u-v;
        if(u<0||v<0||t<0)continue;
        const depth=u*a.z+v*b.z+t*c.z;if(depth< -1||depth>1||best&&depth>=best.depth)continue;
        const weights=[u/a.w,v/b.w,t/c.w],sum=weights.reduce((a,b)=>a+b),p=new THREE.Vector3();
  ...
  prepare(scene:THREE.Scene){
    scene.traverse(object=>{
      ...
      for(const material of Array.isArray(object.material)?object.material:[object.material]){
        ...
        material.userData.nativeView=this;material.fog=false;
        const local={nativeModelScale:{value:object.userData.nativeScale??0},nativeRelative:{value:object.userData.nativeRelative?1:0},nativeObjectBasis:{value:new Int32Array(modelMatrix(0))}};
        if(object.userData.nativeScale)object.onBeforeRender=()=>local.nativeObjectBasis.value.set(modelMatrix(object.parent?.userData.nativeHeading??0));
        material.onBeforeCompile=(shader:Parameters<THREE.Material['onBeforeCompile']>[0])=>{
          ...
          if(object instanceof THREE.Sprite){
            shader.vertexShader=shader.vertexShader.replace('gl_Position = projectionMatrix * mvPosition;',
            ...
            shader.vertexShader=shader.vertexShader.replace(/gl_Position\s*=\s*projectionMatrix\s*\*\s*modelViewMatrix\s*\*\s*vec4\((position|p),\s*1\.?0?\);/g,'gl_Position=nativePosition($1);');
          }else shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>','vec4 mvPosition=modelViewMatrix*vec4(transformed,1.);gl_Position=nativePosition(transformed);');
          shader.fragmentShader=fragment+shader.fragmentShader.replace('void main() {','void main() { if(nativeMode>0.&&!nativeCellVisible())discard;').replace('void main(){','void main(){if(nativeMode>0.&&!nativeCellVisible())discard;');
        };
        material.customProgramCacheKey=()=>object instanceof THREE.Sprite?'native-sprite':'native-mesh';material.needsUpdate=true;
      }
    });
  }
  dispose(){this.boundsTexture.dispose();}
}
