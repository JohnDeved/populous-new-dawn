...
export type CollisionPerson = Point & {
  model: number
  flags2: number
  flags4: number
  tribe: number
...
export type CollisionObject = Point & {
  class: number
  state: number
  flags2: number
  tribe: number
...
export type CollisionWorld = {
  cell: (p: Point) => CollisionCell
  walkMask: ArrayLike<number>
  objects: ReadonlyMap<number, CollisionObject>
  boatAt: (p: Point) => boolean
}
...
export function terrainSupportsPerson(category: number, p: Point) {
  category &= 15
  const flags = rules.terrainCategoryFlags[category]
  if (flags & 1) return 1
  if (flags & 60)
    return (
      rules.terrainCategoryMasks[category][(255 - ((p.y & 510) >> 1)) >> 5] &
      (1 << (7 - ((p.x >> 6) & 7)))
    )
  return 0
}
...
export function restingCellCollision(
  ...
  const x = (p.x >> 8) & 254,
    y = (p.y >> 8) & 254
  for (const bit of [y * 256 + x, y * 256 + x + 1, (y + 1) * 256 + x, (y + 1) * 256 + x + 1])
    if (!(walkMask[bit >> 3] & (1 << (bit & 7)))) return 3
  return 0
}
...
export function buildingBlocksPerson(w: CollisionWorld, p: CollisionPerson, cell: CollisionCell) {
  ...
  if (flags & 2) {
    if (!b) throw new Error(`Missing native building ${id}`)
    return Number(b.state === 1 || p.workTarget !== id)
  }
  if (flags & 4) return Number(p.workTarget !== id)
  if (flags & 0x10000) {
    const target = p.target ? w.objects.get(p.target) : undefined
    if (target && !(target.flags2 & 1) && target.class) {
      const other = w.cell(target).building & 1023
      if (other && other === id) return 0
    }
    return 1
  }
  return 0
...
export function personStepCollision(w: CollisionWorld, p: CollisionPerson, to: Point) {
  const cell = w.cell(to),
    flags = p.flags4,
    category = rules.terrainCategoryFlags[cell.category & 15]
  const passable = () => {
    const bit = ((to.y >>> 8) & 255) * 256 + ((to.x >>> 8) & 255)
    return !!(w.walkMask[bit >> 3] & (1 << (bit & 7)))
  }
  if (!(flags & 0x10007) || p.flags2 & 0x20000000) {
    ...
    if (!(flags & 0x800000) || !(flags & 0x1000000))
      return terrainSupportsPerson(cell.category, to) ? 0 : 4
...
export function pathCellBlocked(
  w: CollisionWorld,
  p: CollisionPerson,
  packed: number,
  heightRange: () => number,
  ...
  if (!(p.flags4 & 0x10007) && cell.flags & 512) {
    ...
    const id = adjacentBuilding()
    return Number(!id || (cell.building & 1023) !== id)
  }
  if (
    cell.flags & 4 ||
    heightRange() > (limit << 16) >> 16 ||
    !(rules.terrainCategoryFlags[cell.category & 15] & 1)
  )
    return 1
