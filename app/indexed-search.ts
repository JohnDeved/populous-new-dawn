...
export function startIndexedSearch(
  pool: Uint8Array,
  type: 1 | 2,
  angle: number,
  first: number,
  ...
  const v = view(pool)
  let id = 1
  while (id < 16 && pool[id * 12]) id++
  if (id === 16) return 0
  const a = id * 12
  pool[a] = 1
  pool[a + 1] = type
  first = Math.min(first & 255, 31)
  last = Math.min(Math.max(last & 255, first), 31)
  pool[a + 2] = first
  pool[a + 3] = last
  v.setInt16(a + 6, 0, true)
  if (type === 1) {
    pool[a + 4] = first
    pool[a + 5] = 3
  } else {
    angle &= 2047
    v.setUint16(a + 4, angle, true)
    const [offset, count] = data.rings[first]
    v.setInt16(a + 8, offset + count * 2, true)
    v.setInt16(a + 10, offset + Math.trunc((count * angle) / 2048) * 2, true)
  }
  return id
}
...
export function nextIndexedSearch(pool: Uint8Array, id: number): { x: number; y: number } | null {
  ...
  let x = 0,
    ...
    valid = true
  if (pool[a + 1] === 1) {
    const radius = pool[a + 4],
      step = v.getInt16(a + 6, true)
    switch (pool[a + 5]) {
      case 0:
        x = step - radius
        y = radius
      ...
      case 3:
        ...
        break
      default:
        throw new RangeError('Invalid native search side')
    }
    ...
    if (v.getInt16(a + 6, true) >= radius * 2) {
      ...
      if (pool[a + 5] > 3) {
        pool[a + 5] = 0
        pool[a + 4]++
        if (pool[a + 4] > pool[a + 3]) valid = false
      }
    }
  } else if (pool[a + 1] === 2) {
    const offset = v.getInt16(a + 10, true)
    x = data.points[offset]
    y = data.points[offset + 1]
    ...
    if (v.getInt16(a + 6, true) > count) {
      ...
      if (pool[a + 2] > pool[a + 3]) valid = false
        ...
        const [start, count] = data.rings[pool[a + 2]],
          angle = v.getInt16(a + 4, true)
        v.setInt16(a + 6, 0, true)
        v.setInt16(a + 8, start + count * 2, true)
        v.setInt16(a + 10, start + Math.trunc((count * angle) / 2048) * 2, true)
      }
    } else {
...
export function endIndexedSearch(pool: Uint8Array, id: number) {
  pool[(id & 255) * 12] = 0
}
