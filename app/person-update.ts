...
type PreparationPerson = StatefulPerson & {
  counter: number
  slowTurn: number
  goalX: number
  goalY: number
...
type HealthPerson = StatefulPerson & {
  h: number
  life: number
  maxLife: number
  healthMarker: number
...
export function finishPersonPreparation(
  p: PreparationPerson,
  effects: Pick<PreparationEffects, 'animation' | 'destination'>
) {
  if (p.slowTurn) {
    p.slowTurn = (p.slowTurn - 1) & 255
    if (!p.slowTurn) effects.animation()
  }
  const flags = p.flags2
  if (flags & 0x80000000) {
    p.flags2 = (flags & 0x7fffffff) >>> 0
    if (!(flags & 128)) effects.destination({ x: p.goalX, y: p.goalY })
  }
}

// Complete 0x4d42a0, before reaction timers, eligibility and physics.
export function preparePersonTurn(
  p: PreparationPerson,
  gameFlags: number,
  ...
  if (rules.personModels[p.model].flags & 64 && p.statusFlags) {
    if (p.statusFlags === 1) {
      if (!(p.counter & 3)) p.statusFlags = 0
    } else p.statusFlags = (p.statusFlags - 1) & 255
  }
  p.flags4 = (p.flags4 & ~0x400000) >>> 0
  resetInterruptedPersonMotion(p)
  if (p.flags2 & 16) {
    resetPersonMotion(p)
    if (!(p.flags2 & 0x100000)) {
      p.previousState = p.state
      p.state = gameFlags & 2 && p.model === 7 ? 39 : rules.personModels[p.model].nextState
      effects.initialize()
...
export function stepPersonReaction(p: {
  class: number
  counter: number
  flags4: number
...
export function personIsDrowning(land: Ground, p: HealthPerson) {
  if (
    p.flags2 & 0x80002 ||
    rules.personPhysicsFlags[p.physics] & 2 ||
    p.h > terrainPointHeight(land, p)
  )
    return false
  const category = land.categories[((p.y & 65535) >> 9) * 128 + ((p.x & 65535) >> 9)]
  if (terrainSupportsPerson(category, p)) {
    p.flags4 = (p.flags4 & ~0x1000000) >>> 0
    return false
  }
  if (!(p.flags4 & 0x800000)) return true
  return (
    !p.vehicle && (!(rules.terrainCategoryFlags[category & 15] & 60) || !(p.flags4 & 0x1000000))
  )
}
