...
function defendingBase(w: SelectionWorld, p: SelectionUnit, command: number | undefined) {
  ...
  const tribe = w.tribes[p.tribe],
    base = tribe.hasBase ? tribe.base : tribe.shaman
...
export function selectComputerPeople(
  ...
  for (let priority = 0; priority < 7; priority++) {
    ...
    for (const p of w.people) {
      ...
      if (
        hut?.class === 2 &&
        rules.buildingFlags[hut.model] & 64 &&
        (hut.inside << 24) >> 24 >= rules.buildingCapacity[hut.model]
      )
        continue
      ...
      const redirect =
        !!(flags & 2) &&
        ([11, 25].includes(command ?? -1) || (!(flags & 16) && defendingBase(w, p, command)))
      if (
        !(
          idle ||
          redirect ||
          (flags & 4 && command === 6) ||
          (flags & 1 && p.assignment & 0x804) ||
          p.flags3 & 1
        )
      )
        continue
