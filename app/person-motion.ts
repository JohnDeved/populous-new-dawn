...
export function turnPerson(p: PersonFacing) {
  ...
  if (p.flags2 & 0x1000 && !(p.flags2 & 0x800)) {
    let target =
      p.flags2 & 128
        ? p.turnAngle
        : p.turnAngle === p.x && p.turnY === p.y
          ? p.heading
          : nativeAngle(short(p.turnAngle - p.x), -short(p.turnY - p.y))
    if (p.slowTurn || !(p.flags2 & 0x200800)) {
      let delta = short(target) - short(p.heading),
        distance = Math.abs(delta)
      if (distance > 1024) {
        distance = 2048 - distance
        delta += delta < 0 ? 2048 : -2048
      }
      let amount = distance
      if (p.slowTurn) amount = Math.trunc(short(distance) / p.slowTurn)
      else if (distance > short(rules.personTurning[p.physics])) {
        amount = short(rules.personTurning[p.physics])
        turning = true
      }
...
export function groundVelocity(
  ...
  if (!speed) return
  angle &= 2047
  const aheadX = short(p.x + ((rules.sine[angle] << 5) >> 16)),
    aheadY = short(p.y + ((rules.sine[(angle + 512) & 2047] << 5) >> 16))
...
export type RecoveryPerson = PersonFacing & {
  h: number
  speed: number
  flags4: number
  motionTimer: number
...
export function stepMotionRecovery(
  p: RecoveryPerson,
  onBuilding: boolean,
  blocks: () => boolean,
  route: (outside: boolean) => { x: number; y: number }
) {
  if (!(p.flags2 & 0x800)) {
  ...
  let finished = false
  if (!(p.flags2 & 0x20000000)) {
    p.motionTimer = short(p.motionTimer - 1)
    finished = p.motionTimer <= 0
  } else if (!onBuilding || (p.flags4 & 0x10007 && !blocks())) {
    p.motionTimer = 0
    finished = true
    ...
    if (p.motionTimer === 0) {
      point = route(false)
      if (
        Math.abs(short(point.x) - short(p.x)) <= 111 &&
        Math.abs(short(point.y) - short(p.y)) <= 111
      ) {
        p.supportHeight = 0
        p.motionTimer++
      }
...
export function recoverGroundObstacle(
  p: RecoveryPerson,
  to: { x: number; y: number; h: number },
  onBuilding: boolean,
  height: (x: number, y: number) => number,
  blocked: (to: { x: number; y: number; h: number }) => number
) {
  p.flags2 = (p.flags2 | 0x800) >>> 0
  if (signedByte(p.recoveryCounter) < 65) {
    if (signedByte(p.motionMode) < 48) p.motionMode = (p.motionMode + 1) & 255
  } else {
    p.motionMode = 0
  ...
  if (onBuilding) {
    p.flags2 = (p.flags2 | 0x20000000) >>> 0
    p.motionTimer = 0
    return true
  }
  for (let pass = 1; pass <= 2; pass++) {
    ...
    for (let attempt = 0; attempt < 11; attempt++, direction = -direction) {
      const angle = (p.heading + 170 * ((attempt >> 1) + 1) * direction) & 2047,
        velocity = { x: 0, y: 0, z: 0 }
      groundVelocity(velocity, p, p.speed * pass, angle, height)
      const point = { x: (p.x + velocity.x) & 65535, y: (p.y + velocity.z) & 65535, h: 0 }
      point.h = height(point.x, point.y)
      if (!blocked(point)) {
        if (p.motionTimer < 1)
          p.motionTimer = short((signedByte(p.motionMode) + 1) * ((attempt >> 1) + 1)) >> 1
        p.heading = angle
        Object.assign(to, point)
        return true
      }
    }
  }
