...
export type StatefulPerson = OrderedPerson & {
  ...
  statusFlags: number; workFlags: number; stateObject: number; speed: number;
  timer: number; target: number; reservationNext: number; formationCell: number;
  cargo: number; animationMode: number; vehicle: number; angle: number; turnAngle: number;
  motionTimer: number; motionMode: number;
};
export type PersonStateWorld = {
  randomState: number; instantFacing: boolean; levelFlags: number; orders: OrderPool;
...
export function personAnimationObject(p: Pick<StatefulPerson, 'state' | 'model' | 'flags2' | 'flags4' | 'speed' | 'cargo'>) {
...
function faceSelection(w: PersonStateWorld, p: StatefulPerson, effects: PersonStateEffects) {
  ...
  if (p.tribe !== -1) {
    ...
    const point = nativeStep({x: tribe.x / 256, z: -tribe.y / 256}, (tribe.angle + 1024) & 2047, 16384);
    const dx = short(Math.round(point.x * 256) - p.x), dy = short(Math.round(-point.z * 256) - p.y);
...
export function initializePersonState(w: PersonStateWorld, p: StatefulPerson, effects: PersonStateEffects) {
  ...
  p.flags2 = (p.flags2 & 0xfcdefddd) >>> 0; p.renderFlags &= 0xfeff;
  if (p.model === 7) { p.flags4 = (p.flags4 | 256) >>> 0; p.statusFlags |= 4; }
  p.flags4 = (p.flags4 & (p.model === 8 ? 0xffff7fff : 0xffff7f7f) & ~8) >>> 0;
  p.assignment &= 0xf51f;
  if (!(stateFlags & 16)) { p.workFlags = 0; p.workTarget = 0; }
  p.flags4 = (p.flags4 & 0xfffefff8) >>> 0;
  if ((p.assignment & 1) && (stateFlags & 4) && !(model.flags & 0x400)) {
    p.assignment &= ~1;
    if (p.state !== 10 || (p.commands[p.commandCursor] & 255)) effects.rebuildFormation(p.formationCell);
  }
  ...
  if (p.tribe !== -1) {
    const tribe = w.tribes[p.tribe];
    if ((tribe.flags & 64) && !(rules.personStateFlags[p.state] & 0x2000)) tribe.flags = (tribe.flags | 1024) >>> 0;
  }
}
...
export function reserveTrainingPerson(w: PersonStateWorld, p: StatefulPerson, effects: PersonStateEffects) {
  if (!(p.flags2 & 0x100000)) {
    p.previousState = p.state; p.state = 14; initializePersonState(w, p, effects);
  }
  p.assignment |= 0x800;
}
