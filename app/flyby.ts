...
export type FlybyEvent = {kind: number; flags: number; value: number; start: number; duration: number};
export type FlybyAxis = {
  acceleration: number; braking: number; cruise: number; velocity: number;
  initial: number; accelerateUntil: number; brakeFrom: number;
};
const short = (n: number) => (n << 16) >> 16;
const f32 = Math.fround;
export const createFlybyAxis = (): FlybyAxis => ({acceleration: 0, braking: 0, cruise: 0, velocity: 0, initial: 0, accelerateUntil: 0, brakeFrom: 0});
...
export function planFlybyAxis(axis: FlybyAxis, distance: number, duration: number) {
  ...
  if (duration === 0 || (distance === -2147483648 && duration === -1)) throw new RangeError('Native flyby division fault');
  ...
  Object.assign(axis, {acceleration, braking, cruise, initial: axis.velocity, accelerateUntil: start, brakeFrom: f32(duration - ramp)});
}

export function stepFlybyAxis(axis: FlybyAxis, position: number, frame: number, duration: number) {
  frame = short(frame); duration = short(duration);
  if (axis.accelerateUntil !== 0 || axis.cruise !== 0) {
    axis.velocity = f32(frame < axis.accelerateUntil ? axis.acceleration * frame + axis.initial :
      frame < axis.brakeFrom ? axis.cruise : (duration - frame) * axis.braking);
    axis.velocity = f32(axis.velocity + .5);
  }
...
export type Flyby = {
  events: FlybyEvent[]; flags: number; cursor: number; warmup: number;
  frameRate: number; averageRate: number; frame: number; end: FlybyCamera | null;
...
export const createFlyby = (): Flyby => ({
  events: [], flags: 16, cursor: 0, warmup: 0, frameRate: 0, averageRate: 0, frame: 0, end: null,
  tracks: Array.from({length: 7}, () => ({frame: 0, duration: 0, active: false})),
  axes: {x: createFlybyAxis(), y: createFlybyAxis(), angle: createFlybyAxis(), zoom: createFlybyAxis()},
});

export function addFlybyEvent(state: Flyby, event: FlybyEvent) {
  if ((state.flags & 1) || state.events.length >= 32) return;
  const e = {kind: event.kind & 255, flags: event.flags & 255, value: event.value & 65535, start: short(event.start), duration: short(event.duration)};
...
export function flybyCommand(state: Flyby, opcode: number, args: number[]) {
  ...
  else if (opcode === 1206) { state.cursor = 0; state.warmup = 6; state.flags = (state.flags & 63) | 1; }
  else if (opcode === 1207) state.flags &= ~1;
  else if (opcode === 1208) state.flags = a ? state.flags | 16 : state.flags & ~16;
  else if (opcode === 1214) { state.flags |= 4; state.end = {x: a & 255, y: b & 255, angle: short(c), zoom: short(d)}; }
  else if (opcode === 1209 || opcode === 1212) addFlybyEvent(state, {kind: opcode === 1209 ? 1 : 4, flags: 0, value: packed, start: c, duration: d});
  else if (opcode === 1210 || opcode === 1211) addFlybyEvent(state, {kind: opcode === 1210 ? 2 : 3, flags: 0, value: opcode === 1210 ? a : Math.trunc((short(a) << 8) / -100), start: b, duration: c});
  else if (opcode === 1213) addFlybyEvent(state, {kind: 5, flags: c === 0 ? 1 : c === 1 ? 2 : c, value: packed, start: d, duration: e});
  else if (opcode === 1215) addFlybyEvent(state, {kind: 6, flags: 0, value: a, start: b, duration: 1});
...
function beginEvent(state: Flyby, camera: FlybyCamera, event: FlybyEvent) {
  ...
  if (kind === 1) {
    ...
    if (magnitude > 1024) { magnitude = 2048 - magnitude; direction = -direction; }
    const forced = (event.flags << 24) >> 24;
    if (forced && Math.sign(forced) !== direction && direction) { direction = -direction; magnitude = (2048 - magnitude) & 2047; }
...
export function stepFlyby(state: Flyby, camera: FlybyCamera, measuredFrameRate: number, paused = false) {
  ...
  while (!(state.flags & 8) && state.cursor < state.events.length && state.events[state.cursor].start <= short(state.frame)) {
  ...
  for (let kind = 1; kind <= 6; kind++) {
    ...
    const step = (axis: FlybyAxis, position: number) => stepFlybyAxis(axis, position, track.frame, track.duration);
    if (kind === 1) { camera.x = step(state.axes.x, camera.x); camera.y = step(state.axes.y, camera.y); }
    if (kind === 2) camera.angle = step(state.axes.angle, camera.angle) & 2047;
    if (kind === 3) camera.zoom = Math.max(-16384, Math.min(16384, step(state.axes.zoom, camera.zoom)));
    if (kind >= 5 || track.frame >= track.duration) track.active = false;
    track.frame = short(track.frame + 1);
  }
  state.frame = short(state.frame + 1);
  if ((state.flags & 8) && state.tracks.every(t => !t.active)) { state.flags &= ~1; camera.zoom = 0; }
...
export function interruptFlyby(state: Flyby, camera: FlybyCamera) {
  ...
  if (state.warmup) { state.flags &= ~1; camera.zoom = 0; return; }
  ...
  for (const [kind, value] of [[1, end.x | (end.y << 8)], [2, end.angle], [3, end.zoom]]) {
