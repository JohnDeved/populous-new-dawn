...
import {terrainSlopeRange,terrainDrift,terrainPointHeight,hasNonLand,type NativeTerrain} from './native-terrain.ts';
...
import {buildingBlocksPerson,personStepCollision,type CollisionWorld,type CollisionPerson} from './person-collision.ts';
...
export function bouncePerson(w:CollisionWorld,p:CollisionPerson & {velocity:Velocity},to:Position,reflect:boolean){
  const cell=w.cell(to);
  if(!(cell.flags&0x80200)||((p.flags4&0x10007)&&(cell.flags&0x200)&&!buildingBlocksPerson(w,p,cell)))return;
  const dx=p.velocity.x>0?-8:8,dy=p.velocity.z>0?-8:8;
  let x=to.x,y=to.y,previousX=x,steps=0;
  do{
    // A fully blocked wrapped diagonal also never terminates in the executable.
    if(steps++===8192)throw new Error('Native bounce has no unblocked cell on its wrapped diagonal');
    previousX=x;x=(x+dx)&65535;y=(y+dy)&65535;
  }while(w.cell({x,y}).flags&0x80200);
  if(reflect){const axis=(previousX&0xfe00)===(x&0xfe00)?'z':'x';p.velocity[axis]=short(-p.velocity[axis]);}
  to.x=x;to.y=y;
}

export type PhysicsPerson=RecoveryPerson & SettlingPerson & CollisionPerson & {life:number};
type Point=Position & {h:number};
export type PhysicsWorld={land:Ground;collision:CollisionWorld;gameFlags:number;levelFlags:number;playerTribe:number;
  buildingModel:(id:number)=>number;route:(outside:boolean)=>Position};
export type PhysicsEffects=SettleEffects & {
  // 0x4ee580 owns cell lists and updates the person's stored position.
  insert:(to:Point)=>boolean;allocate:(unitClass:number,model:number,tribe:number,to:Point)=>void;
  sound:(cue:number)=>void;damage:(attacker:number,amount:number,mode:number)=>void;
  reveal:(radius:number,cell:number)=>void;path:()=>void;
};
...
export function stepPersonPhysics(w:PhysicsWorld,p:PhysicsPerson,effects:PhysicsEffects,mask=0){
  if(p.flags2&0x4000)return;
  const to={x:p.x,y:p.y,h:p.h},v=p.velocity,physics=p.physics;
  const height=(x:number,y:number)=>terrainPointHeight(w.land,{x,y});
  const move=()=>{to.x=(to.x+v.x)&65535;to.y=(to.y+v.z)&65535;to.h=short(to.h+v.y);};
  const clampHeight=()=>{to.h=Math.max(to.h,height(to.x,to.y));};
  const decay=(value:number,amount:number)=>short(Math.abs(value)<amount?0:value<=0?value+amount:value-amount);
  const fall=()=>{
    p.flags4=(p.flags4|0x400)>>>0;
    if(!(p.flags2&0x40000))v.y=short(v.y-short(rules.personGravity[physics]));
    v.x=decay(v.x,2);v.z=decay(v.z,2);limitPersonVelocity(p.physics,v);move();
  };
  const landingEffect=()=>effects.allocate(7,3,p.tribe,{...to});
  if(p.flags2&0x2000){
    limitPersonVelocity(p.physics,v,true);move();v.y=short(v.y-short(rules.personGravity[physics]));
    if(!(rules.personPhysicsFlags[physics]&4))bouncePerson(w.collision,p,to,false);
    if(!(p.flags2&2))clampHeight();effects.insert(to);
  }
  if(!(p.flags2&0x80000)){
    const turning=turnPerson(p);
    if(height(to.x,to.y)<to.h){
      fall();bouncePerson(w.collision,p,to,true);
      if(to.h<=height(to.x,to.y)){p.flags2=(p.flags2|0x1000)>>>0;p.flags4=(p.flags4&~0x400)>>>0;}
    }else{
      if(p.flags4&0x400)landingEffect();p.flags4=(p.flags4&~0x400)>>>0;
      let speed=p.speed;
      const destination={x:p.turnAngle,y:p.turnY};
      if((p.flags2&0x200)&&!(p.flags2&128)&&positionsOverlap(p,56,destination,1024)){
        const dx=short(destination.x-p.x),dy=short(destination.y-p.y),distance=(dx*dx+dy*dy)|0;
        if(distance<speed*speed)speed=Math.floor(Math.sqrt(distance>>>0));
      }
      if(speed){
        const cell=w.collision.cell(p),onBuilding=!!(cell.flags&0x200);
        stepMotionRecovery(p,onBuilding,()=>!!buildingBlocksPerson(w.collision,p,cell),w.route);
        v.x=v.z=0;if(v.y<0)v.y=0;
        groundVelocity(v,to,speed,short(p.heading),height);limitPersonVelocity(p.physics,v);move();to.h=height(to.x,to.y);
        ...
        if(blocked(to)){
          ...
          else if(!recoverGroundObstacle(p,to,onBuilding,height,blocked)&&unsupportedGround(w.land,p,p.physics,mask))p.flags2=(p.flags2|0x80000)>>>0;
        }
      }
    }
  }else{
    if(height(to.x,to.y)<to.h){
      ...
      fall();p.heading=(p.heading+0x93)&2047;p.angle=p.heading;
    }else{
      const landed=!!(p.flags4&0x400);p.flags4=(p.flags4&~0x400)>>>0;
      if(landed){
        if(v.y<-99&&!(rules.terrainCategoryFlags[w.collision.cell(to).category&15]&2)){
          ...
          if(p.life>0){if(p.model===7)effects.sound(0x1b);else if(p.model!==1)effects.sound(9);}
        }
        if(p.class===1)effects.animation();landingEffect();
      }
      const drift=terrainDrift(w.land,to);v.x=short(v.x+drift.x);v.y=short(v.y+drift.y);v.z=short(v.z+drift.z);
      const bit=(to.y>>>8)*256+(to.x>>>8);
      if(w.land.walkMasks[mask][bit>>3]&(1<<(bit&7))){
        const friction=short(rules.personFriction[physics][rules.terrainCategoryFlags[w.collision.cell(to).category&15]&0x3e?1:0]);
        v.x=decay(v.x,friction);v.z=decay(v.z,friction);
      }
      limitPersonVelocity(p.physics,v);move();p.heading=nativeAngle(short(to.x-p.x),-short(to.y-p.y));
      p.angle=p.flags2&0x8000?(p.heading+1024)&2047:p.heading;
    }
    bouncePerson(w.collision,p,to,true);settlePerson(w.land,p,to,w.gameFlags,w.collision.objects,effects,mask);
  }
  if(!(p.flags2&2))clampHeight();
  const moved=effects.insert(to);
  if(p.supportHeight){
    const cell=w.collision.cell(p);
    if(!(cell.flags&0x200)||!rules.buildingSupportHeight[w.buildingModel(cell.building&1023)])p.supportHeight=0;
  }
  if((w.levelFlags&4)&&moved&&p.tribe===w.playerTribe){
    const heading=p.heading&2047,dx=(rules.sine[heading]<<9)>>16,dy=(rules.sine[(heading+512)&2047]<<9)>>16;
    let x=p.x,y=p.y;
    for(let i=0;i<5;i++,x=(x+dx)&65535,y=(y+dy)&65535){
      if(!(w.collision.cell({x,y}).flags&8)){effects.reveal(2,((p.x>>>8)&254)|(p.y&0xfe00));break;}
    }
  }
  effects.path();
}
