...
export type ComputerTask = {
  flags: number; type: number; phase: number; target: number; requested: number;
  extra: number; selected: number; remaining: number; elapsed: number; mode: number;
};
export type ComputerQueue = {
  tasks: ComputerTask[]; cursor: number; flags: number; selectionOwner: number; commandDelay: number;
};

export function createComputerQueue(): ComputerQueue {
  return {tasks: Array.from({length: 10}, () => ({flags: 0, type: 0, phase: 0,
    target: 0, requested: 0, extra: 0, selected: 0, remaining: 0, elapsed: 0, mode: 0})),
    cursor: 0, flags: 0, selectionOwner: 10, commandDelay: 0};
}
...
export function requestTraining(ai: ComputerQueue, count: number, model: number,
  available: number, firstBuilding: (model: number) => number) {
  ...
  Object.assign(task, {flags: ((task.flags & ~2) | 1) >>> 0, type: 6,
    target: target | 0, requested: count | 0, extra: 0, phase: 0});
...
function acquireSelection(ai: ComputerQueue, index: number) {
  const task = ai.tasks[index];
  if (!(ai.flags & 2)) {
    if (task.type !== 20 && ai.tasks.some((other, i) => i !== index &&
      (other.flags & 1) && other.type === 20 &&
      (task.mode === 0 ? [2, 7, 11, 14] : [3, 15]).includes(other.phase))) return false;
...
export type TrainingBuilding = {
  id: number; owner: number; state: number; model: number; capacity: number;
  inside: number; occupants: ({id: number; model: number} | null)[];
};
export type TrainingInput = {
  tribe: number; preference: number; population: number; trained: number;
  committed: number; maximum: number;
  select: (building: TrainingBuilding, count: number) => number[];
};
export type TrainingAction = {kind: 'restore'} |
...
export function stepTrainingTask(ai: ComputerQueue, index: number,
  building: TrainingBuilding | null, input: TrainingInput) {
  const task = ai.tasks[index], actions: TrainingAction[] = [];
  const cleanup = () => {
  ...
  if (!building || (task.flags & 2)) { cleanup(); return actions; }
  switch (task.phase) {
    case 0:
    case 2: {
      task.remaining = task.requested;
      if (!task.requested) {
        const desired = (Math.trunc(Math.imul(input.preference & 255, input.population) / 100) -
          ((input.trained << 16) >> 16) - input.committed) | 0;
    ...
    case 4: {
      ...
      if (!people.length) {
        if (task.selected) { task.phase = 5; ai.commandDelay = 20; }
        else { releaseSelection(ai, index); actions.push({kind: 'restore'}); task.phase = 8; }
      } else {
        ...
        if (task.remaining < 1) { task.phase = 5; ai.commandDelay = 20; }
      }
      break;
    }
    case 5: task.phase = 6; break;
    case 6:
      releaseSelection(ai, index);
      if (building.owner === input.tribe && building.state !== 1) {
        actions.push({kind: 'train', id: building.id});
        task.phase = 7; task.elapsed = 0;
      } else { actions.push({kind: 'restore'}); task.phase = 8; }
      break;
    case 7:
