...
The original first mission, rebuilt for desktop browsers with stylized low-poly models and a spherical world. Its terrain, initial objects and worship timings were imported from the user-supplied game.
...
`app/model.ts` owns simulation; `app/scene.ts` owns Three.js/input/camera; `app/page.tsx` owns the HUD. `app/level-one.ts` is generated by `scripts/import-level.py`. Original game models, textures, music, voices and executable code are not shipped.
