...
test('ordinary groups settle into native rings, compact on departure and retain status through new orders',()=>{
  for(const count of [1,2,3,4,5,6,12]) {
    ...
    assert.equal(unitAnimationSource(u),person,'native movement retains original sprite ownership')
    assert.equal(person.object,40,'brave walking uses the original walk row')
    for(let i=0;i<200;i++)tick(w,1/12)
    rings(w)
