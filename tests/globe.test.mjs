...
import {beginGlobeMorph,stepGlobeMorph} from '../app/camera-view.ts'
...
test('world-view terrain matches captured native mesh order, projection and lighting', () => {
  ...
  for (const { view, triangles, sha256 } of [...fixture.meshes,...fixture.morphMeshes]) {
...
test('overview morph timing matches the original complete lifetime snapshots', () => {
  for (const c of fixture.morphSequences) {
    const morph = structuredClone(c.initial)
    beginGlobeMorph(morph,c.entering)
    const states = [structuredClone(morph)]
    while (morph.active) {
      stepGlobeMorph(morph)
      states.push(structuredClone(morph))
    }
    assert.equal(createHash('sha256').update(JSON.stringify(states)).digest('hex'),c.sha256)
  }
})
