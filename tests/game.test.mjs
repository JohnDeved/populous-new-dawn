...
test('mission-one Dakini launches its later building attack when Blue overwhelms it',async()=>{
 ...
 assert.equal(task.target,packed(buildingPosition(buildingPose(blueBuildings[1]))));assert.equal(task.entity,blueBuildings[1].id);
 assert.equal(direct.randomState,1896349699,'building selection consumes one native draw');
 const fallback=createWorld();fallback.buildings=[];fallback.units=fallback.units.filter(u=>u.kind!=='shaman');fallback.randomState=2;
 ...
 assert.equal(task.target,packed(nativePosition(fallback,person)),'an empty building list falls back to allocation-ordered people');assert.equal(task.entity,person.id);
...
test('mission-two enemy marker orders nearby Matak defenders to counterattack',()=>{
 const world=createWorld(2);until(world,()=>world.turn>=122,20);
 const tower=world.buildings.find(b=>b.team==='green'&&b.kind==='tower'),
  shaman=world.units.find(u=>u.team==='blue'&&u.kind==='shaman');
 assert.ok(tower&&shaman);
 const defender=addUnit(world,'green','warrior',{x:tower.x+2,z:tower.z});
...
test('Mission 5 grants, boards, sails, lands and checkpoints its original Boat',async()=>{
 ...
 const before=[boat.x,boat.y];assert.ok(command(w,browserPosition(target)));until(w,()=>boat.x!==before[0]||boat.y!==before[1],3);assert.ok(followers.every(u=>u.native?.vehicle===boat.id));assert.ok(followers[0].native.motionGroup);assert.ok(followers.every(u=>u.native.motionGroup===followers[0].native.motionGroup));assert.notDeepEqual([boat.x,boat.y],[target.x,target.y]);
 until(w,()=>followers.every(u=>!u.native?.vehicle),20);assert.deepEqual(boat.passengers,[]);assert.equal(boat.speed,-1);assert.ok(vehicleReady(vehicleWorld(),boat));assert.ok(followers.every(u=>vehicleCanDisembark(vehicleWorld(),boat,u.native)));
