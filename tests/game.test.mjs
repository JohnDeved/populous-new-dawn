...
test('mission-one victory continuation creates and restarts the recovered mission-two world', async () => {
 ...
 const tower=world.buildings.find(b=>b.team==='red'&&b.kind==='tower');assert.ok(world.ai.tasks.some(t=>t.flags&1&&t.type===7&&t.target===tower.id));
 until(world,()=>world.units.some(u=>u.inside===tower.id&&u.team==='red'&&u.kind==='warrior'),4);
 until(world,()=>world.turn>=70,4);assert.equal(world.inputMask&128,0);assert.equal(world.messages.slots[world.lastMessage].stringId,641);assert.equal(world.messages.slots[world.lastMessage].flags&0x20000,0x20000);
 ...
 const patrol=world.units.filter(u=>u.team==='red'&&u.native&&currentPersonOrder(world.buildingOrders,u.native)?.model===25);assert.equal(patrol.length,3);assert.ok(patrol.every(u=>u.native.computerAssignment===0));
