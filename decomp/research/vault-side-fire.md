...
`00408840` is the concrete side-flame producer. It selects the current rotated building shape, walks all six triples at shape-record offsets 26..43, and for each triple with any nonzero byte allocates `alloc_unit(5, 10, owner, point)`. Those class-5/model-10 objects are the ordinary fire effect. The existing `scripts/check-native-building-fire.py` already executes `00408cb0 -> 00408840` and `00408ab0` against the original executable and compares attachment placement and burn-phase dispatch. Allocation, people, sound, plan ownership and lifecycle callbacks are intercepted; fire-object lifetime has a separate checker. This helper comparison is not a complete native gameplay or final-raster proof. `references/reverse-engineering.md` records the same six-triple contract.
...
The existing [building-fire checker](../../scripts/check-native-building-fire.py) documents its executable input and intercepted callbacks; reuse it without `--record` when new native execution is needed. The [importer](../../scripts/import-original.py) defines the original 48-byte shape record, six fire triples at byte 26, and object-to-shape indices. The [descriptor extractor](../../scripts/inspect-executable.py) defines the model flags address. Retained [ignition](../generated/00408cb0.c), [attachment producer](../generated/00408840.c), [Vault task](../generated/0043c7a0.c), and [sink handler](../generated/00407060.c) provide the selected control-flow evidence. No new native execution was needed for this note; imported data conclusions were checked against the hash-pinned original files.
