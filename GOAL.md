...
| AI and campaign | Original first-level layout; native script control flow CPU-compared, turn-zero setup, disabled Dakini reincarnation and native marker-based head removal applied | Complete game-command bindings and AI scheduler; all missions, objectives, progression and difficulty |
| Persistence and multiplayer | Not implemented | Original save/load behavior and multiplayer simulation/protocol behavior |
| Validation | Thirteen regression tests and real-browser mission/visual QA | Original-engine traces, cross-engine replay comparisons, complete campaign and multiplayer scenarios |
