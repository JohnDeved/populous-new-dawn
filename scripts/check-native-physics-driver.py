...
Cell insertion/list splices also execute natively. Allocation, damage/audio, state/fight, building routes, reveal and
...
fields={'id':(0x24,'H'),'cellNext':(0x20,'H'),'cellPrevious':(0x22,'H'),'x':(0x3d,'H'),'y':(0x3f,'H'),'h':(0x41,'h'),'class':(0x2a,'B'),'model':(0x2b,'B'),'physics':(0x30,'B'),'state':(0x2c,'B'),'previousState':(0x7d,'B'),'flags2':(0xc,'I'),'flags3':(0x14,'I'),'flags4':(0x10,'I'),'motionTimer':(0x61,'h'),'motionMode':(0x66,'B'),'target':(0x72,'H'),'tribe':(0x2f,'B'),'workTarget':(0x89,'H'),'life':(0x6e,'h'),'counter':(0x2e,'B'),'turnAngle':(0x57,'H'),'turnY':(0x59,'H'),'heading':(0x5d,'H'),'angle':(0x26,'H'),'slowTurn':(0x7e,'B'),'speed':(0x5f,'h'),'recoveryCounter':(0x65,'B'),'supportHeight':(0x1c,'h')}
def put(person):
 cpu.mem_write(p,bytes(256))
 for k,(offset,f) in fields.items():write(p+offset,f,person[k])
 write(p+0x49,'hhh',*person['velocity'].values());write(p+0x43,'hhh',*person['displacement'].values())
def get():return {**{k:read(p+offset,f) for k,(offset,f) in fields.items()},'velocity':dict(zip(['x','y','z'],struct.unpack('<hhh',cpu.mem_read(p+0x49,6)))),'displacement':dict(zip(['x','y','h'],struct.unpack('<hhh',cpu.mem_read(p+0x43,6))))}
...
for id,o in objects:
 o.update(id=id,h=0,flags3=0,cellNext=id-1,cellPrevious=id+1 if id<3 else 0,displacement=dict(x=0,y=0,h=0));o['flags2']|=0x20000
...
write(0x890390+4*4,'I',p)
touched={0}
consumers={0x4ed8a0:'allocate',0x4da080:'damage',0x48a050:'sound',0x4ed6f0:'release',0x4ed640:'initialize',0x4d3ea0:'animation',0x4465a0:'class3',0x51f990:'fight',0x51fcd0:'ready',0x450610:'reveal',0x4eadc0:'path',0x40a460:'approach',0x4044b0:'outside',0x4665c0:'boat'}
def leaf(cpu,a,size,u):
 sp=cpu.reg_read(UC_X86_REG_ESP);name=consumers[a];args=[];result=0
 if name=='allocate':args=[read(sp+4,'B'),read(sp+8,'B'),read(sp+12,'B'),point(read(sp+16,'I'))]
...
def observe_insert(cpu,a,size,u):
 sp=cpu.reg_read(UC_X86_REG_ESP);q=point(read(sp+8,'I'));touched.update([index(get()),index(q)]);events.append(['insert',[q],get()])
cpu.hook_add(UC_HOOK_CODE,observe_insert,begin=0x4ee580,end=0x4ee580)
...
def person(i):
 ...
 return dict(id=4,cellNext=0,cellPrevious=0,displacement={k:rng.randrange(-32768,32768) for k in ['x','y','h']},x=x,y=y,h=h,**{'class':rng.choice([1,1,3])},model=rng.randrange(1,9),physics=i%20,state=rng.choice([10,36,39,41]),previousState=14,
  flags2=rng.choice([0,0x1000,0x1800,0x4000,0x80000,0x80000,0x82000,0x2000,0x20000800])|rng.choice([0,2,32,0x200,0x8000,0x40000]),flags3=rng.choice([0,0x10000,0x8000000,0x8010000])|rng.choice([0,0x100,0x200,0x300]),flags4=rng.choice([0,0x400,0x2000,0x2400])|rng.choice([0,1,2,4,0x10000,0x1800000]),
...
js="""import {insertObjectIntoCell,moveObjectInCells} from './app/object-cells.ts';import {createHash} from 'node:crypto';import {stepPersonPhysics,bouncePerson} from './app/person-physics.ts';
const land={...input.land,walkMasks:[new Uint8Array(8192)]},index=q=>((q.y&65535)>>9)*128+((q.x&65535)>>9),short=n=>(n<<16)>>16;
console.log(JSON.stringify(input.cases.map(c=>{const objects=new Map(structuredClone(input.objects)),p=c.p,events=[],log=(n,...a)=>events.push([n,a,structuredClone(p)]);
const cells={heads:new Uint16Array(16384),objects};cells.heads[0]=3;objects.set(p.id,p);insertObjectIntoCell(cells,p,p);
...
const effects={insert:to=>{log('insert',{...to});return moveObjectInCells(cells,p,to);},allocate:(...args)=>log('allocate',...args),damage:(...args)=>{log('damage',...args);p.life=short(p.life-args[1]);},sound:cue=>log('sound',cue),animation:()=>log('animation'),release:()=>log('release'),initialize:()=>log('initialize'),class3:()=>log('class3'),canFight:id=>{log('fight',id);return c.fight;},readyToFight:()=>{log('ready');return c.ready;},reveal:(...a)=>log('reveal',...a),path:()=>log('path')};
...
const turns=[];for(let i=0;i<c.turns;i++){events.length=0;stepPersonPhysics(world,p,effects);turns.push({p:structuredClone(p),events:structuredClone(events),cells:createHash('sha256').update(new Uint8Array(cells.heads.buffer)).digest('hex'),links:[1,2,3].map(id=>{const o=objects.get(id);return [o.cellNext,o.cellPrevious];})});p.counter=(p.counter+1)&255;}
return turns;})));"""
for mode,count,turns in [('bounce',4096,1),('driver',8192,1),('trajectory',128,64)]:
 ...
 for i in range(count):
  ...
  case['buildingModel']=[4,13,15][i%3];objects[0][1]['model']=case['buildingModel'];write(p+256+0x2b,'B',case['buildingModel']);put(pp)
  for cell in touched:write(0x8a03e4+cell*16+6,'H',0)
  touched={0,index(pp)};write(0x8a03e4+6,'H',3)
  for id,o in objects:write(p+id*256+0x20,'HH',o['cellNext'],o['cellPrevious'])
  write(to,'HHh',pp['x'],pp['y'],pp['h']);call(0x4ee470,p,to)
  cpu.mem_write(0x96aaba,bytes([255 if case['passable'] else 0])*8192);write(0x89d17c,'I',case['gameFlags']);write(0x895da8,'I',case['levelFlags']);write(0x89c6f0,'B',0)
  events=[]
  if mode=='bounce':
   ...
   for turn in range(turns):
    events=[];call(0x4e6d00,p);memory=bytes(cpu.mem_read(0x8a03e4,16384*16));output.append(dict(p=get(),events=copy.deepcopy(events),cells=hashlib.sha256(b''.join(memory[j:j+2] for j in range(6,16384*16,16))).hexdigest(),links=[[read(p+id*256+0x20,'H'),read(p+id*256+0x22,'H')] for id in [1,2,3]]));write(p+0x2e,'B',(read(p+0x2e,'B')+1)&255)
