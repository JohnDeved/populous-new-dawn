...
const baseline='9dbfb2fcfe8454c9537ff8661eb99acb08649f44'
...
try {
 ...
 for(let i=0;i<100;i++){
  ...
  for(const mode of i%2?[1,0]:[0,1]){
   ...
   if(i>=20)samples[mode].push(performance.now()-start)
  }
 ...
 const stats=a=>{a.sort((x,y)=>x-y);return {median:a[a.length/2],p95:a[Math.floor(a.length*.95)]}}
 console.log(JSON.stringify({baseline,cpu:cpus()[0].model,node:process.version,people:200,buildings:12,warmups:20,pairedSamples:80,milliseconds:{before:stats(samples[0]),after:stats(samples[1])},scope:'Complete group input CPU only; no rendering/GPU/FPS claim. Person records, orders, marching and RNG equal after every paired input.'}))
