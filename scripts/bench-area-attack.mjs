...
const blocked = process.argv.includes('--blocked'), samples = []
let checksum
for (let pass = 0; pass < 5; pass++) {
  ...
  w.terrainVersion++
  const hut = addBuilding(w, 'red', 'hut', { x: blocked ? 20 : 0, z: 0 })
  for (let i = 0; i < 200; i++) addUnit(w, 'blue', 'warrior', blocked ? { x: -20 + i % 20 / 5, z: Math.floor(i / 20) / 5 } : { x: i % 20 / 5, z: 20 + Math.floor(i / 20) / 5 })
...
const report = { runtime: process.version, cpu: cpus()[0].model, blocked, units: 200, samples,
  limits: 'Five deterministic 600-turn replays of 200 attackers approaching one hut and continuing to a waypoint. Blocked mode connects two islands at turn 120. Includes live simulation, command input and target scans; no rendering. First replay includes JIT warmup. Dense arrival crowd remains subject to native route/retry limits. No hardware FPS or before/after speedup claim.' }
writeFileSync(`references/performance/2026-09-11-${blocked ? 'route-recovery' : 'area-attack'}-cpu.json`, JSON.stringify(report, null, 2) + '\n')
