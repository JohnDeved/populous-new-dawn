...
const preset = process.argv.includes('--close') ? 3 : 0
const cases = [],
  reportState = {}
try {
  ...
  const terrainState = await page.evaluate(async () => {
    ...
    return {
      turn: s.world.turn,
      geometrySha256: [...new Uint8Array(hash)].map(n => n.toString(16).padStart(2, '0')).join(''),
    }
  })
  ...
  for (const [width, height] of [
    ...
    await page.waitForFunction(
      ({ width, height }) => {
        const s = window.testScene,
          side = document.querySelector('.native-hud').getBoundingClientRect().width
        return (
          s.renderer.domElement.width === width - side && s.renderer.domElement.height === height
        )
      },
      { width, height }
    )
    for (const center of [
      ...
      for (const heading of [0, 256, 512, 768, 1024, 1280, 1536, 1792]) {
        const result = await page.evaluate(
          async ({ center, heading, width, modern, preset }) => {
            const s = window.testScene,
              r = s.renderer,
              gl = r.getContext()
            const { polygonMeshBounds, cameraPreset, cameraConfigIndex } =
            ...
            s.view.update(
              ...
              width,
              preset
                ? cameraPreset(cameraConfigIndex(width, s.container.clientHeight), preset)
                : undefined
            )
            const expandedViewport = s.view.bounds.map(row => [...row])
            const native = polygonMeshBounds(s.view.config.bounds, heading)
            const images = [],
              draws = []
            for (const padding of modern ? [0, 16, 32, -1] : [0, 16, 32]) {
              s.view.bounds =
                padding === -1
                  ? expandedViewport
                  : padding
                    ? polygonMeshBounds(
                        s.view.config.bounds.map((n, i) =>
                          i % 2 ? n : n + Math.sign(n) * padding
                        ),
                        heading
                      )
                    : native
              ...
              draws.push({
                padding,
                cells: window.coverageTerrain.geometry.drawRange.count / 6,
            ...
            const nativePixels = images[0].reduce(
              (count, value, i) => count + Number(i % 4 === 0 && value >= 128),
              0
            )
            return {
              heading,
              ...
              ...(modern
                ? {
                    modern: compare(images[0], images[3]),
                    modernVsReference: compare(images[2], images[3]),
                  }
                : {}),
            }
          },
          { center, heading, width, modern, preset }
        )
        assert.ok(result.nativePixels > 0, 'Native terrain must render in the isolated scene')
        assert.ok(
          result.draws.every(draw => draw.calls === 1 && draw.triangles === draw.cells * 18)
        )
...
const report = {
  ...
  scope:
    'Normal/close preset recorded separately; isolated white terrain through live native shaders. Original nine terrain copies retained. Padding 16/32 is a diagnostic reference; padding -1 is the current viewport expansion. Triangle counts measure submitted work, not GPU time or FPS.',
  preset,
  terrainState: reportState,
  cases,
}
const output =
  process.argv.find(a => a.endsWith('.json')) ?? '/private/tmp/populous-ground-coverage.json'
writeFileSync(output, JSON.stringify(report, null, 2) + '\n')
console.log(
  `Recorded ${cases.length} coverage comparisons to ${output}; no whole-display parity claim.`
)

if (modern)
  for (const c of cases) {
    assert.equal(c.modern.removed, 0, JSON.stringify(c))
    assert.deepEqual(c.modernVsReference, { added: 0, removed: 0 }, JSON.stringify(c))
  }
