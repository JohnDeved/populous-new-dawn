...
try{
  ...
  const controls=page.locator('.tribe-classes button'),total=page.getByRole('button',{name:'Select follower',exact:true})
  ...
  for(const c of cases){
    ...
    await page.evaluate(c=>{
      ...
      s.world.units=[...original.filter(u=>u.team!=='blue'||u.kind==='shaman'),...Array.from({length:c.count},(_,i)=>({...template,id:20000+i,kind,native:undefined,entry:undefined,builder:undefined}))]
      s.world.selected=[];s.onChange()
    },c)
    const button=c.model===0?total:page.getByRole('button',{name:`Select ${c.model===2?'brave':'warrior'}`,exact:true})
    ...
    if(c.selected){await button.click({modifiers:['Shift']});await page.mouse.move(1200,900);await page.waitForFunction(()=>[...document.querySelectorAll('.tribe-classes button')].some(b=>b.getAttribute('aria-pressed')==='true'))}
