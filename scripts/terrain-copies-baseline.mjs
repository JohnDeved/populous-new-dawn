...
export async function installTerrainCopiesBaseline(page) {
  await page.evaluate(async () => {
    ...
    const s = window.testScene, update = s.view.updateTerrainVisibility
    window.selectTerrainCopies = optimized => {
      s.view.updateTerrainVisibility = optimized ? update : RenderView.prototype.updateTerrainVisibility
      if (!optimized) s.scene.traverse(object => {
        if (!object.isInstancedMesh || !object.userData.terrainGrid) return
        const matrix = object.matrixWorld.clone()
        terrainTiles.forEach(([x, z], i) => object.setMatrixAt(i, matrix.makeTranslation(x * 256, 0, z * 256)))
