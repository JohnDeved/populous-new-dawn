...
COM/cache/allocation are supplied. Original format selection, palette/AL
conversion, tile traversal, edge passes and sprite wrappers execute unchanged.
"""
...
palette_object, palette_table, get_entries, format_ptr = 0x2400000, 0x2401000, 0x2ffb200, 0x2402000
convert, unlocks = False, 0
...
def adapter(c, address, size, data):
    global cursor, allocations, unlocks
    if address == 0x42f640 and convert:
        return
    ...
    if address == 0x487380:
        ...
        if convert:
            assert read(sp + 20, 'I') == format_ptr
        ...
        if convert:
            c.mem_write(descriptor + 72, bytes(c.mem_read(format_ptr, 32)))
        cleanup = 20
    elif address == unlock:
        unlocks += 1
        cursor += int(not convert or unlocks % 2 == 0)
        cleanup = 8
    elif address == get_entries:
        assert read(sp + 16, 'I') == 256
        c.mem_write(read(sp + 20, 'I'), palette)
        cleanup = 20
...
for address in [0x4574d0, 0x487380, 0x42f4f0, 0x42f640, 0x55aca0, lock, unlock, get_entries]:
...
# Complete format selector, including its separate cutout and blended paths.
for capabilities in range(8):
    for formats in range(8):
        for blend_supported in [0, 1]:
            opaque = 0x2402100 if formats & 1 else 0
            cutout = 0x2402200 if formats & 2 else 0
            blended = format_ptr if formats & 4 else 0
            write(ui + 0x6c8, 'I', capabilities)
            write(ui + 0xd08, 'I', blend_supported)
            write(ui + 0x1da0, 'I', opaque)
            write(0xafc2f8, 'I', cutout)
            write(0xafc2e8, 'I', blended)
            write(stack, 'I', stop)
            cpu.reg_write(UC_X86_REG_ESP, stack)
            cpu.reg_write(UC_X86_REG_ECX, ui)
            cpu.emu_start(0x5218d0, stop, count=10000)
            assert cpu.reg_read(UC_X86_REG_EIP) == stop
            for mode_at, texture_at, preference in [(0x6cc, 0x1dac, [4, 2, 1]), (0x6d0, 0x1da8, [2, 4, 1])]:
                mode = next((v for v in preference if capabilities & v), 0)
                if mode_at == 0x6cc and not blend_supported and capabilities & 1:
                    mode = 1
                texture = (cutout or blended) if mode_at == 0x6cc else (blended or cutout)
                if mode not in [2, 4] or not texture:
                    mode, texture = capabilities & 1, opaque
                assert read(ui + mode_at, 'I') == mode
                assert read(ui + texture_at, 'I') == texture
print('PASS: 128 complete native format selections, capability flags, format availability and cutout override')
...
# Run the full initializer again from actual indexed source pixels. Only device
# services are supplied; conversion, format-mask decoding and edge loops execute.
...
prepared = assets.object_texture(raw)
palette = inputs[1]
cpu.mem_write(0x87f000, inputs[2])
cpu.mem_write(0x2500000, inputs[0])
write(palette_object, 'I', palette_table)
write(palette_table + 16, 'I', get_entries)
write(ui + 0x6c0, 'I', palette_object)
write(ui + 0x6c8, 'I', 7)
write(0xafc2e8, 'I', format_ptr)
write(format_ptr, '8I', 32, 0x41, 0, 16, 0xf00, 0xf0, 0xf, 0xf000)
write(stack, 'I', stop)
cpu.reg_write(UC_X86_REG_ESP, stack)
cpu.reg_write(UC_X86_REG_ECX, ui)
cpu.emu_start(0x5218d0, stop, count=10000)
assert read(ui + 0x1da8, 'I') == format_ptr and read(ui + 0x6d0, 'I') == 2
for tile in range(256):
    write(0x974160 + tile * 4, 'I', 0x2500000 + (tile // 8) * 8192 + (tile % 8) * 32)
    cpu.mem_write(pixels + tile * 4096, bytes(4096))
convert, cursor, allocations, unlocks, pitch, alpha_mask = True, 0, 0, 0, 32, 0xf000
write(stack, 'I', stop)
cpu.reg_write(UC_X86_REG_ESP, stack)
cpu.emu_start(0x4b6e60, stop, count=30000000)
assert cpu.reg_read(UC_X86_REG_EIP) == stop and cpu.reg_read(UC_X86_REG_EAX) == 0
assert cursor == allocations == 256 and unlocks == 512
for tile in range(256):
    words = struct.unpack('<1024H', cpu.mem_read(pixels + tile * 4096, 2048))
    for y in range(32):
        for x in range(32):
            offset = (((tile // 8) * 32 + y) * 256 + (tile % 8) * 32 + x) * 4
            assert rgba(words[y * 32 + x]) == prepared[offset:offset + 4], (tile, x, y)
print('PASS: all 262,144 actual object texels through native ARGB4444 selection, palette/AL conversion and edge preparation')
atlas = ROOT / 'public/original/atlas.png'
assert Image.open(atlas).convert('RGBA').tobytes() == prepared
quantized = bytes((v >> 4) * 17 for v in raw)
changed = [i for i in range(0, len(raw), 4) if quantized[i:i + 4] != prepared[i:i + 4]]
assert changed and all(raw[i + 3] == prepared[i + 3] == 0 for i in changed)
assert raw[3::4] == prepared[3::4]
samples = []
opaque = [i for i in range(0, len(raw), 4) if prepared[i + 3] and raw[i:i + 3] != prepared[i:i + 3]]
probes = changed[::max(1, len(changed) // 32)][:32] + opaque[::max(1, len(opaque) // 32)][:32]
for i in probes:
    x, y = i // 4 % 256, i // 4 // 256
...
fixture = dict(executableSha256=identity['sha256'], atlasSha256=hashlib.sha256(atlas.read_bytes()).hexdigest(),
               format='ARGB4444', changedTexels=len(changed), quantizedVisibleTexels=len(opaque), samples=samples)
...
print(f'PASS: source-bound atlas, {len(opaque)} quantized visible and {len(changed)} prepared transparent texels; every alpha byte unchanged')
