...
for n in range(1024):
 ...
 before=order(rng.choice([0,3,21,21]),rng.randrange(65536));before['flags']=rng.randrange(256);before['object']=rng.randrange(65536)
 if n%3==0:before.update(model=21,a=a,b=b)
 ...
 call(0x438730,10,21,base+0x4000,flags)
 result=dict(zip(before.keys(),struct.unpack('<BB4H',cpu.mem_read(0x938830+100,10))))
 prepared.append(dict(before=before,area=dict(a=a,b=b),category=category,flags=flags,expected=result))
js2="""
import {prepareCombatOrder} from './app/person-orders.ts';let text='';for await(const c of process.stdin)text+=c;
console.log(JSON.stringify(JSON.parse(text).map(c=>{const categories=new Uint8Array(16384),a=c.area.a;categories[((a>>>9)&127)*128+((a&254)>>1)]=c.category;prepareCombatOrder(c.before,c.area,c.flags,categories);return c.before})))
"""
...
print('PASS: 1024 complete command-21 preparation calls, all coastal categories, wrapped centers and unchanged-record flag retention.')
