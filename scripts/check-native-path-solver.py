...
const w={state,path,result,categories,flags,walkMasks,boatsEnabled:c.enabled,landLimit:32,computerLimit:c.searchLimit,humanLimit:c.searchLimit,tribes:[{playerType:1}],cellObjects:cell=>c.startBoat&&cell===c.cell?[{id:2,class:4,model:1}]:[]},e={buildingAccess:cell=>{events.push(['building',cell]);return c.building;},boardingBoat:cell=>{events.push(['boarding',cell]);return c.boarding;},disembark:(boat,p)=>{events.push(['disembark',boat,p.x,p.y]);return !!c.disembark;},boatCell:(cell,boat)=>{events.push(['boatCell',cell,boat]);return !!c.boatCell;}};let value=null;
