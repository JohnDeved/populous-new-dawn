...
def leaf(cpu,address,size,user):
    ...
    if address==0x4d9580:actions.append(['routeRecovery'])
    elif address==0x4d6f90:actions.append(['idleApproach'])
    elif address==0x4d7330:actions.append(['resting'])
    elif address==0x518480:actions.append(['encounter'])
    elif address==0x5184e0:actions.append(['fight'])
...
hooks={a:cpu.hook_add(UC_HOOK_CODE,leaf,begin=a,end=a) for a in [0x4d9580,0x518480,0x5184e0,0x4d4040,0x409580,0x4d56f0,0x4ea460,0x432260,0x4e0af0,0x4dfac0,0x4d6f90,0x4d7330,0x4d3250,0x4d80e0,0x40a3f0]}
...
for trial in range(8704):
    ...
    if trial>=8192:u.update(state=33,vehicle=0)
...
print('PASS: 8704 native state-1/8/10/14/17/19/21/25/26/29/33/36/39/41/44 initializers, flags, RNG, selection counts and camera-relative facing; world effects supplied')
