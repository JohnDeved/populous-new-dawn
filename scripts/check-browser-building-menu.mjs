...
try {
  ...
  const openFixture = async id => {
    await page.evaluate(id => {
      const s = window.testScene
      const building = s.world.buildings.find(candidate => candidate.id === id)
      if (!building) throw new Error(`fixture building ${id} is missing`)
      // Each controlled fixture must be in view, including after occupant focus
      // moved the camera during the preceding case.
      s.focus(building)
      s.updateView()
      if (!s.visible(building)) throw new Error(`fixture building ${id} is outside the current view`)
