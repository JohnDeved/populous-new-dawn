...
const w={state,path,result,categories,boatsEnabled:c.enabled,landLimit:c.landLimit,computerLimit:c.computerLimit,humanLimit:c.humanLimit,tribes:[{playerType:c.playerType}],cellObjects:cell=>cell===c.cell?c.objects:[]};let si=0;
...
for mode in ['boat','collect','search','composed']:
 ...
 for n in range(2048):
  ...
  for k,(a,f) in fields.items():
   ...
   write(a,f,value)
  cpu.mem_write(p,bytes(256));write(p+0x24,'H',1);write(p+0x2f,'b',0);write(p+0x9f,'H',ps['vehicle']);write(p+0x10,'I',ps['flags4']);write(0x938830,'B',c['command']);write(0x89d1c8+0xc1f,'B',c['playerType']);write(0x89bc7e,'I',c['enabled']);write(0x5aa450,'I',c['landLimit']);write(0x89ce5c,'B',c['computerLimit']);write(0x89ce5e,'B',c['humanLimit'])
