...
Deletion/fight creation, planned destinations, cell insertion and sound are supplied
consumers. Original building geometry, entry/wait, ejection and outdoor logic run.
Initial allocation/state entry remains separate.
"""
...
load_native_shapes(cpu,exe,0x2020000,0x2023000)
building=0x2000600
...
def consumer(c,a,size,user):
 ...
 if a==0x4e9d80:
  p=read(sp+4,'I');point=read(sp+8,'I');cpu.mem_write(p+0x4f,bytes(cpu.mem_read(point,4)))
 elif a==0x4ee580:
  p=read(sp+4,'I');point=read(sp+8,'I');cpu.mem_write(p+0x3d,bytes(cpu.mem_read(point,6)))
 elif a==0x4edcf0:outcome='cancelled'
...
for a in [0x4edcf0,0x51de60,0x48a050,0x4e9d80,0x4ee580]:cpu.hook_add(UC_HOOK_CODE,consumer,begin=a,end=a)
rules=json.loads((ROOT/'app/original-rules.json').read_text());cases=[];expected=[]
occupied=[]
for n in range(8192):
 for cell in occupied:write(0x8a03e4+cell*16,'I',cell&1)
 occupied=[]
 ...
 for id_,address in [(1,attacker),(2,defender)]:
  ...
  if n>=4096:p.update(substate=rng.choice([1,2,3]),counter=rng.randrange(256),commandPhase=rng.randrange(256),assignment=rng.choice([0,16]),workTarget=0,animationMode=0)
 ...
 b=None
 if n>=4096:
  b=dict(id=4,**{'class':2 if n%19 else 0},flags2=1 if n%29==0 else 0,object=131+n%3,angle=(n%4)*512,anchorX=x&0xfe00,anchorY=y&0xfe00)
  cpu.mem_write(building,bytes(256));write(0x890390+4*4,'I',building)
  for key,off,f in [('id',0x24,'H'),('class',0x2a,'B'),('flags2',12,'I'),('object',0x33,'H'),('angle',0x26,'H'),('anchorX',0x7a,'H'),('anchorY',0x7c,'H')]:write(building+off,f,b[key])
  if n&1:
   occupied=list(set((p['y']>>9)*128+(p['x']>>9) for p in people))
   for cell in occupied:write(0x8a03e4+cell*16,'I',(cell&1)|512)
 seed=rng.getrandbits(32);flags=64 if n%5==0 else 0;activity=n%3
 write(0x89d167,'B',activity)
 cpu.mem_write(group,bytes(256));write(group+0x6a,'2H',1,2);write(group+0x68,'H',4 if b else 0);write(0x89d178,'I',seed);write(0x89d17c,'I',flags)
 ...
 cases.append(dict(people=people,randomState=seed,gameFlags=flags,playerTribe=0,musicActivity=activity,building=b,occupied=occupied.copy()))
...
const outcome=stepMeleeEncounter(w,...c.people,{animation:(p,object)=>setPersonAnimation(p,object,{playerTribe:0,gameFlags:c.gameFlags,sessionSubstate:null,tribes:Array.from({length:4},()=>({flags:0,playerType:0})),objects:new Map()},sprites),height:(x,y)=>terrainPointHeight(land,{x,y}),sound:(p,cue)=>sounds.push([p.id,cue]),building:{destination:(p,to)=>{p.goalX=to.x;p.goalY=to.y;},move:(p,to)=>Object.assign(p,to,{h:terrainPointHeight(land,to)}),occupied:p=>c.occupied.includes((p.y>>9)*128+(p.x>>9))}});
...
print('PASS: 8192 original outdoor/building encounter controller calls, poses/timers/facing, speed and impulse RNG, slope velocity, ordered sound, music activity and completion/cancellation.')
if '--record' in sys.argv:
 (ROOT/'tests/fixtures/melee-encounter.json').write_text(json.dumps(dict(heights=heights,cases=[dict(input=cases[i],expected=expected[i]) for i in range(0,8192,29)]),separators=(',',':'))+'\n')
