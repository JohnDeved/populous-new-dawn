...
Usage: python SCRIPT EXE [--departure] [--record]
"""
...
departure='--departure' in sys.argv
entry=0x497690 if departure else 0x4958f0
...
for a in [0x4e9d80,0x4e9dd0,0x4d4040,0x48a050,0x4eec80,0x4ea460]:cpu.hook_add(UC_HOOK_CODE,hook,begin=a,end=a)
...
cases=[];expected=[]
for model in ([1,2,3,4,5,7,13,14] if departure else [1,2,3,4,5,7]):
 for phase in ([0,2,3,4,5,6,18,19,21,255] if departure else [0,1,3,4,23,54,255]):
  for trial in range(64):
   ...
   task=dict(task=9 if departure else 2,busy=rng.choice([0,1,2,255]),phase=phase,restart=bool(rng.randrange(4)==0))
   ...
   expected.append(dict(person=result,task=dict(task=9 if departure else 2,busy=read(p+0xaa,'B'),phase=read(p+0xa8,'B'),restart=restart),randomState=read(0x89d178,'I'),events=events))
js="""import {stepBuildingWork,stepBuildingDeparture} from './app/building-work.ts';import {setDirectPersonDestination,createMotionRoutes} from './app/person-routes.ts';
...
(task.task===9?stepBuildingDeparture:stepBuildingWork)(rng,p,task,c.site,{destination:(to,direct)=>{events.push(['destination',to,direct]);if(direct)setDirectPersonDestination(createMotionRoutes(),p,to);else{p.goalX=to.x;p.goalY=to.y}},releaseMotion:()=>events.push(['releaseMotion']),animation:(_,id)=>events.push(['animation',id]),rest:()=>events.push(['rest']),sound:(cue,flags)=>events.push(['sound',cue,flags])});
...
if '--record' in sys.argv:
 ...
 (ROOT/('tests/fixtures/building-departure.json' if departure else 'tests/fixtures/building-work.json')).write_text(json.dumps(dict(executableSha256=identity['sha256'],cases=[cases[i] for i in indexes],expected=[expected[i] for i in indexes]),separators=(',',':'))+'\n')
print(f'PASS: {len(cases)} complete native {"departure" if departure else "building-work"} controllers: all phases, geometry, movement speed/RNG, readiness, facing, poses, sound and signed wait boundaries')
