...
- `levl2001.hdr` selects AI script 10, so the matching campaign file is **levels/cpscr010.dat**, not the similarly named expansion script. Its bytecode is now imported by `scripts/import-script.py`; the reconstructed interpreter runs turn-zero initialization. The recurring terrain-marker/head-removal, opening/discovery messages and flyby, spell-entry shutoff and warrior-dependent attack-commitment blocks execute through the browser interpreter. The remaining attack/marker orders and AI scheduler are unfinished.
...
This recreates the first mission's geography and progression, not the original executable. Combat damage, recoil animation, breeding/upgrade timing, training cost and reincarnation delay use a compact browser simulation. Enemy AI still defends locally and casts Blast. Native script initialization supplies its state/attribute and spell-defense setup and disables Dakini reincarnation. The recurring script disables its two Blast entries once the allocation count exceeds one; browser defensive casting now respects those entries. Target scoring, mana eligibility and casting cadence still differ from the original. Spell charging uses normalized mana units and an initial stock of four Blasts. Land raising is gradual after a native-style projectile flight, but its deformation rule remains approximate. Normal view uses the recovered native camera projection. The overview remains approximate, and the full 128 × 128 map is not yet simulated. There is no fog-of-war, saving, multiplayer or later-campaign content. Models, textures and Web Audio cues remain original.
