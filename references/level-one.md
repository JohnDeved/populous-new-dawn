...
- `levl2001.hdr` selects AI script 10, so the matching campaign file is **levels/cpscr010.dat**, not the similarly named expansion script. Its bytecode is now imported by `scripts/import-script.py`; the reconstructed interpreter runs turn-zero initialization. Recurring commands still need their game-side bindings.
...
- Mission flow and special events: [The Journey Begins](https://wiki.popre.net/The_Journey_Begins), [level-one walkthrough](https://ts.popre.net/websites/tsh/info_walkthroughs_level1.html). Worship, bridge to the centre, defeat the guard, learn warrior training, gather Lightning gifts, bridge to the enemy. The southern head becomes inactive once the two tribes are connected. Victory checks remaining followers rather than requiring every empty building to be demolished.
