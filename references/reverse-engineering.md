...
`scripts/check-native-worship.py` compares 4,824 actual native worship turns across follower interruptions, varying required counts, growth, refill, exhaustion and negative/unlimited counts. Three real `004facf0` initializations and 18 subsequent visits establish frames 1077/1059/1068 for Vault/Lightning/Land Bridge, ground plus 800 height, the linked neutral frame-1417 glow at minus 80 height, and visibility for phases 6 through 1 before phase 0 hides the reward and deletes the glow. Another 1,660 actual reward-processor turns compare the browser's 82-turn delivery delay and both packed stock counters, including full/over-cap stocks and saturation. Worship fixtures provide real command records and cell lists; the native eligibility routine executes. Reward allocation, sunlight and deletion consumers are supplied, and a non-player recipient excludes the Windows UI callback. The executable identity is verified before execution; no executable bytes are embedded in the check.

The browser now creates those automatic rewards in the existing effect collection, reuses the original unit atlas for the main icon and glow, hides both after the sixth object visit, and preserves the existing 82-turn grant. `scripts/check-browser-worship.mjs` verifies live GPU pixels, frames, terrain offsets, phase/timer progression, pause behavior, delivery and cleanup for all three mission-one rewards.

Limits: the browser still supplies approximate order/radius eligibility and a shared world phase instead of native per-object phases. Native worship chooses the largest eligible tribe group; current first-mission worship orders and rewards are player-only, without competitive tribe ownership. Vault movement/adjacency leaves remain browser adapters. Generic command-29 pickup, competitive ownership and Windows UI callbacks remain unported. These comparisons establish automatic mission reward presentation and delivery under the stated fixtures, not full trigger, reward or campaign parity.
