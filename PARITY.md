...
- **partial** — Dynamic lighting and shadows (`graphics.lighting`). Terrain diffuse/additive conversion, live scenery shade and original airborne person shadow art/grounding/scaling are compared and integrated. Sunlight propagation, model lighting, full object shadow ownership/interpolation and scenery lifecycle remain incomplete. Evidence: [scripts/check-native-vertex-lighting.py](scripts/check-native-vertex-lighting.py), [scripts/check-browser-water.mjs](scripts/check-browser-water.mjs), [scripts/check-native-scenery-shadow.py](scripts/check-native-scenery-shadow.py), [scripts/check-browser-scenery-fire.mjs](scripts/check-browser-scenery-fire.mjs), [tests/game.test.mjs](tests/game.test.mjs), [scripts/check-native-unit-shadows.py](scripts/check-native-unit-shadows.py), [scripts/check-browser-unit-shadows.mjs](scripts/check-browser-unit-shadows.mjs).
