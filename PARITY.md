...
- **partial** — Directional unit sprite selection across all states (`graphics.sprites`). Eight-way selection and native body depth buckets/scaling integrated; complete action/state ownership and render-position interpolation remain open. Evidence: [scripts/check-native-animation.py](scripts/check-native-animation.py), [app/scene.ts](app/scene.ts), [scripts/check-native-unit-shadows.py](scripts/check-native-unit-shadows.py), [scripts/check-browser-unit-shadows.mjs](scripts/check-browser-unit-shadows.mjs).
