...
- **partial** — Dynamic lighting and shadows (`graphics.lighting`). Terrain diffuse/additive conversion, live scenery shade, airborne person shadows and model sunlight normals/shades/depth fading are compared and integrated. Dynamic sunlight ownership/propagation, colored highlights, debris lighting and complete object/scenery lifecycles remain incomplete. Evidence: [scripts/check-native-vertex-lighting.py](scripts/check-native-vertex-lighting.py), [scripts/check-browser-water.mjs](scripts/check-browser-water.mjs), [scripts/check-native-scenery-shadow.py](scripts/check-native-scenery-shadow.py), [scripts/check-browser-scenery-fire.mjs](scripts/check-browser-scenery-fire.mjs), [tests/game.test.mjs](tests/game.test.mjs), [scripts/check-native-unit-shadows.py](scripts/check-native-unit-shadows.py), [scripts/check-browser-unit-shadows.mjs](scripts/check-browser-unit-shadows.mjs), [scripts/check-native-model-lighting.py](scripts/check-native-model-lighting.py), [scripts/check-browser-model-lighting.mjs](scripts/check-browser-model-lighting.mjs).
