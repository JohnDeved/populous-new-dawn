...
- **partial** — Dynamic lighting and shadows (`graphics.lighting`). Terrain diffuse/additive conversion and scenery shadow updates are verified and integrated for live trees, including removal. Sunlight propagation, model lighting, unit shadows and the full scenery lifecycle remain incomplete. Evidence: [scripts/check-native-vertex-lighting.py](scripts/check-native-vertex-lighting.py), [scripts/check-browser-water.mjs](scripts/check-browser-water.mjs), [scripts/check-native-scenery-shadow.py](scripts/check-native-scenery-shadow.py), [scripts/check-browser-scenery-fire.mjs](scripts/check-browser-scenery-fire.mjs), [tests/game.test.mjs](tests/game.test.mjs).
